...
# expId:    jul12-019  (idea-16 Arc-3 = jul12-018 + 择时·防御·绝对动量 切国债)
# baseExpId: jul12-018  (ETF 20日动量 top3 baseline; TRAIN score -0.1917, sharpe -0.45 DQ, annual -3%)
# harness:  research/harness.md (冻结成本 OVERRIDE 见文末; 迭代只跑 --window train)
# design:   ~13 只跨资产 ETF -> 20 日动量 top3; 再对每个入选槽位做绝对动量过滤:
#           若该 ETF 自身 20 日动量<=0 则该槽换成国债 511010(权重聚合, 防御性集中);
#           等权 total_value/3 每槽. 511010 恒在池内故动量必被取到. run_weekly(weekday=1, open).
# filters:  ETF 池: 剔除停牌 ETF; §3 股票概念(ST/次新/涨跌停)对 ETF N/A. 上市<250自然日(截至2022前)
#           在 initialize 里用 get_security_info 校验并剔除, NaN 历史再兜底剔除.
# note:     Arc-3 目标: 过 sharpe>=2.5 (baseline DQ -0.45, annual -3%, maxdd 16%). 观察 annual 是否转正
#           (避险路由躲开 least-bad-loser 陷阱) 且 maxdd 守住~16% -> sharpe 跳向 2.5.
...
def set_params(context):
    ...
    g.mom_window = 20       # 20 日动量：close[-1]/close[-21]-1
...
def rebalance(context):
    ...
    top = list(ranked.index[:g.hold_count])
    if not top:
        return

    # 择时·防御·绝对动量：入选槽位自身 20 日动量<=0 -> 换国债 511010（权重聚合）
    BOND = '511010.XSHG'
    bond_ok = BOND in mom.index                    # 国债动量可用（未停牌/历史充足）
    each = 1.0 / g.hold_count
    weights = {}
    for s in top:
        if mom.get(s, 0.0) <= 0:
            dest = BOND if bond_ok else None       # 债不可用则该槽转现金
        else:
            dest = s
        if dest is None:
            continue
        weights[dest] = weights.get(dest, 0.0) + each

    # 先卖出不在目标里的持仓（停牌不强卖）
    for s in list(context.portfolio.positions.keys()):
        if s not in weights and not cd[s].paused:
            order_target_value(s, 0)

    # 再按聚合权重买入
    total = context.portfolio.total_value
    for s, w in weights.items():
        order_target_value(s, total * w)
