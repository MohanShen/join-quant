...
# expId:    jul12-013  (idea-10 NEW baseline: 最小市值周度轮动 min-cap weekly rotation)
# baseExpId: (none — fresh strategy, like jul12-000 was; NOT a mutation of jul12-005)
# harness:  research/harness.md (冻结成本 OVERRIDE 见文末; 迭代只跑 --window train)
# design:   纯净可变异基线. universe_size=400 (市值升序最小) -> hold_count=10 等权;
#           run_weekly(weekday=1, open). 无日内择时/止盈/破开盘价/handle_data tick 循环.
# filters:  §3 强制真实性 — 剔除 ST/*ST/退, 停牌, 次新(<250自然日), 涨停开盘不买/跌停不卖;
#           另剔除 688* (科创板) 与 30* (创业板 300/301).
# note:     ESTABLISH 新弧的参考 objective(TRAIN); 无 bar to beat. gate sharpe>=2.5 仍计算.
