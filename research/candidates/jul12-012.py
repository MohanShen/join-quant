...
# expId:    jul12-012  (idea-9 = jul12-005 + 止盈阈值 profit_pct>=5 -> >=6)
# baseExpId: jul12-005  (target_count=6, obj(TRAIN)=1.3898)
...
# mutation: 风控·止盈阈值 单因子: stop_profit() 14:50 止盈线 profit_pct>=5 -> >=6
#           (让赢家多跑). 涨停保留 guard + sellable_amount 检查保持不变. 无池/真实性变化 -> runtime-safe.
# note:     bar to beat obj(TRAIN) > 1.3898. jul12-011(>=4) annual升但maxdd升(净-0.018); 此测另一侧.
#           梯度预测: 更少止盈触发->更慢换手->更低maxdd. 证伪: 若过夜carry反使maxdd升则+5%即峰.
