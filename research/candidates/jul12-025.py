...
# expId:    jul12-025  (idea-22 NEW baseline Arc-5: Arc-1 引擎移植到中证1000)
# baseExpId: jul12-005  (Arc-1 finalized winner; 引擎 BIT-IDENTICAL, TRAIN obj 1.3898 / sharpe 5.96)
# source:   strategies/2026-06-21_小市值低开优化-10年年化近100_回撤15-7a1c225f.py  -> [[7a1c225f_小市值低开优化]]
# harness:  research/harness.md (冻结成本 OVERRIDE 见文末; 迭代只跑 --window train)
# change:   唯一结构改动 = get_stocks 股票池: 「全A最小市值」-> index_stocks('000852.XSHG') 中证1000,
#           池内保持市值升序. 日内引擎(低开gap/振幅<=10/MA5/破开盘价11:30/止盈+5%@14:50/target_count=6/
#           avoid_months[1,4]/buy1>5万流动性底/frozen OVERRIDE)全部 BIT-IDENTICAL, 干净归因.
# note:     ESTABLISH Arc-5 参考 objective(TRAIN); 无 bar to beat. gate sharpe>=2.5 仍计算.
#           对比 Arc-1 jul12-005 (TRAIN 1.3898): 检验日内反转 edge 在流动中证1000上是否存活(vs 微盘退化).
#           次新<250d: 中证1000成分股按指数编制天然非次新, §3 由指数成员资格结构性满足.
#           ⚠ 中证1000 日内剥头皮零滑点下 objective 仍偏高估(相对微盘减弱但非零, harness §2).
...
def initialize(context):
    ...
    g.target_count = 6         # 目标持仓数量（最多同时持有6只股票）
