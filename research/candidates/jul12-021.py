...
# expId:    jul12-021  (idea-18 Arc-3 反转探针 = jul12-018 + 排名方向翻转: 买 20d 最差 bottom3)
...
# design:   ~13 只流动跨资产 ETF 池 -> 每只 20 日动量 close[-1]/close[-21]-1 -> 升序取 bottom3(最差)等权;
#           run_weekly(weekday=1, open). 直接检验反转假设(动量在此窗口是否反向可预测).
...
def rebalance(context):
    ...
    ranked = mom.sort_values(ascending=True)      # 动量升序：买最差 bottom3（反转探针）
