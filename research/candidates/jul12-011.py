...
# expId:    jul12-011  (idea-8 = jul12-005 + 止盈阈值 profit_pct>=5 -> >=4)
...
# mutation: 风控·止盈阈值 单因子: stop_profit() 14:50 止盈线 profit_pct>=5 -> >=4
#           (更早锁盈). 涨停保留 guard + sellable_amount 检查保持不变. 无池/真实性变化 -> runtime-safe.
# note:     bar to beat obj(TRAIN) > 1.3898. +4%截断本会跑过+5%的右尾赢家, 观察 annual 是否下滑(证伪路径).
