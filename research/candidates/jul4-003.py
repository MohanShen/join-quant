...
# —— 风控 SLOT（风控因子：止损线）——
# 变异(jul4-003)：个股止损线——持仓浮亏 ≤ g.stop_loss 即清仓。
# 源：[[止损模块]]；[[小市值因子]]「止损强度↔回撤：联合止损<单止损线<无止损」。
def risk_exits(context):
    outs = []
    for s, pos in context.portfolio.positions.items():
        if pos.avg_cost > 0 and (pos.price / pos.avg_cost - 1) <= g.stop_loss:
            outs.append(s)
    return outs
