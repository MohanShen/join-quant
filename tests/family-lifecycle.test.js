...
test('family matching proposes and abstains', async t => {
  ...
  // ⚠ A SCAFFOLD is a legal state now. wiki-family-build.js creates pages from nothing (its
  // `base:` is the literal placeholder `[[<postId8>_<代表基类>]]`), and /run-family fills them in.
  // These invariants belong to a POPULATED page; asserting them over every file turns "this
  // family has not been researched yet" into a test failure, which says nothing useful.
  const realSkipped = skipped.filter(n => !scaffold(n));

  await t.test('every POPULATED family page has a readable base', () => {
    assert.deepStrictEqual(realSkipped, [],
      `families with an unreadable base that are not scaffolds: ${realSkipped.join(', ')}`);
  });
  ...
  await t.test('a strategy matching several bases abstains as a combination',
               { skip: bases.size < 2 ? 'needs >=2 populated bases' : false }, () => {
...
test('family lineage', async t => {
  ...
  await t.test('五福闹新春 is recorded as a sub-lineage of ETF动量',
               { skip: scaffold('五福闹新春') ? 'page is an unfilled scaffold' : false }, () => {
    ...
    // ⚠ This link is NOT regenerated by wiki-family-build.js — it is knowledge, not derivation.
    // A rebuilt 五福闹新春 page must have it re-added by hand or by the研究 loop.
    assert.strictEqual(par.get('五福闹新春'), 'ETF动量');
  });

  await t.test('a parent and child count as the same lineage, in both directions', () => {
    // Test the FUNCTION with an explicit map rather than whatever the wiki happens to declare —
    // the relation is what is under test, not today's taxonomy.
    const m = new Map([['五福闹新春', 'ETF动量']]);
    assert.ok(familyMatch.sameLineage('五福闹新春', 'ETF动量', m));
    assert.ok(familyMatch.sameLineage('ETF动量', '五福闹新春', m));
  });
...
/** A family page created by wiki-family-build.js and not yet filled in by /run-family. */
const scaffold = name => {
  try {
    return /^base:\s*\[\[<postId8>/m.test(
      fsx.readFileSync(pathx.join(__dirname, '../wiki/families', name + '.md'), 'utf8'));
  } catch { return false; }
};
