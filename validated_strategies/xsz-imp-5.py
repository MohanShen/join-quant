...
# expId:            xsz-imp-5
# ideaId:           imp-5  (= study q-e6-10: rank the final candidates by market cap alone)
# baseExpId:        xsz-q-e6-0  (= base 7a1c225f + epoch-6 OVERRIDE, study/小市值/baseline-e6.py)
# family:           小市值   (source base: strategies/2026-06-21_小市值低开优化-10年年化近100_回撤15-7a1c225f.py)
# mutation:         one line — valid_stocks.sort(key=买一额/市值, reverse=True) removed, so candidates keep
#                   the preselect order (market cap ascending). Built by study/小市值/build-e6.js.
# train_objective:  1.5155   (TRAIN 2022-01-01..2023-12-31: total 596.93% / annual 164.35% / maxDD 12.80% / sharpe 6.43; base 1.3861)
# val_objective:    1.4274   (VAL   2024-01-01..2025-12-31: total 563.38% / annual 157.56% / maxDD 14.82% / sharpe 5.60)
# sharpe_val:       5.60
# gate_val:         pass     (sharpe 5.60 >= 1.5; pre-registered pass line obj >= 0.5 and sharpe >= 1.5)
# harness_epoch:    6
# ranAt:            2026-09-22
#
# flags:
#   ⚠⚠ 零滑点高估 —— microcaps × near-daily turnover × zero slippage. ABSOLUTE LEVELS ARE NOT REALIZABLE.
#      epoch-2 study-q-1 measured ~−20% objective at the author's own 0.2% PriceRelatedSlippage.
#   ⚠⚠ daily-frequency bench — the author's 9:30:30 / 11:30 / 14:50 times are approximated by JQ's day engine.
#   ⚠  VAL is clean for OUR change (selected on TRAIN only), NOT for the author's parameters: g.choice 1000 and
#      the −1% gap both sit on a hump top (q-e6-6..9) and the author's ten-year backtest very likely covers 2024–25.
#   ⚠  VAL's deepest drawdown is 2024-05→06, not the 2024-02 microcap crash; the January no-buy rule may have left
#      the book in cash into February. That rule is inert on TRAIN (q-e6-4). Unexamined.
#   Full write-up: wiki/families/小市值.md §1/§2/§6 (q-e6-0..12, imp-1..5, val-imp-5).
# ================================================================================
