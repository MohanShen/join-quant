# ===== VALIDATED STRATEGY (Agent 4 archive) — join-quant autoresearch epoch 2 =====
# expId:            jul12-023
# ideaId:           idea-20  (Arc-4 市场中性: 微盘多头 − IC 中证500 空头 @1.2× 对冲比)
# baseExpId:        jul12-022  (Arc-4 baseline: 微盘多头 − IC 空头 beta≈1.0×; TRAIN 0.2641 DQ)
# train_objective:  0.3202   (TRAIN 2022-01-01..2023-12-31: annual 44.29% / maxdd 12.27% / sharpe 2.64, gate PASS)
# val_objective:    -0.5260  (VAL   2024-01-01..2024-12-31: total/annual -11.00% / maxdd 41.60% / sharpe -0.39, gate FAIL)
# sharpe_val:       -0.39
...
#   (雪球/DMA/量化去杠杆)重创微盘远超中证500 -> IC 空头未保护 -> maxdd 41.6%。TRAIN 过门槛是
#   REGIME-SPECIFIC: 2022-23 熊市 IC 与微盘同跌(对冲对齐), 该同向性在 2024 破裂。并非它表面上的
#   regime-agnostic 市场中性。
# ⚠ UN-COSTED FUTURES: VAL objective(已深负)仍偏乐观 — 真实 IC 对冲成本会使其更差。薄 TRAIN
#   门槛边际(2.64, 仅超 2.5 门槛 0.14)在窗口外反转。
# ================================================================================

# Research candidate — join-quant autoresearch (epoch 2)
# expId:    jul12-023  (idea-20 Arc-4 gap-closer = jul12-022 + 对冲比 1.0x -> 1.2x)
# baseExpId: jul12-022  (市场中性 微盘多头 − IC 空头 beta≈1; TRAIN score 0.2641, sharpe 2.14 DQ, maxdd 13.3%)
...
#           -> 最小 10 只等权, run_weekly 调仓. 空头对冲(子仓1): 做空 IC 主力合约, 名义规模≈1.2×多头
#           市值 (提高对冲比以削残余正 beta), 主力换月时滚动. Factors §2.1: 选股·规模价值[小市值];
...
def rebalance(context):
    ...
    if price is None or price <= 0 or long_value <= 0:
        ...
        contracts = int(round(1.2 * long_value / notional_per))   # 对冲比 1.2×
