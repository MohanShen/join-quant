# ===== VALIDATED STRATEGY (Agent 4 archive) — join-quant autoenhance epoch 6 =====
# expId:            etfmom-005
# ideaId:           idea-1  (满仓小市值腿 + 1/4月日历规则; 本文件另叠加 enable_dynamic_position 后继步)
# baseExpId:        etfmom-004  (= base 22152780 + [1,0,0,0] + avoid_trade_april=True)
# family:           ETF动量   (source base: strategies/2026-06-21_无未来函数_模拟运行了俩月_小有成效-22152780.py)
# mutation:         相对 base 三行 —— L138 g.portfolio_value_proportion [0.5,0,0.5,0] -> [1,0,0,0]
#                                   L169 g.avoid_trade_april       False -> True
#                                   L186 g.enable_dynamic_position False -> True
#                   + 冻结的 epoch-6 成本 OVERRIDE (本纪元首次钉死 stock 侧 set_order_cost)
# train_objective:  0.6516   (TRAIN 2022-01-01..2023-12-31: total 210.94% / annual 76.47% / maxDD 11.31% / sharpe 3.20 / gate pass)
# val_objective:    1.3973   (VAL   2024-01-01..2025-12-31: total 544.14% / annual 153.80% / maxDD 14.07% / sharpe 5.27)
# sharpe_val:       5.27
# gate_val:         pass     (sharpe 5.27 >= 1.5, epoch-6 validate 门槛)
# status:           recorded (confirmed: VAL 过闸且与 TRAIN 同向 —— 仅在下列 flags 的限定内成立)
# harness_epoch:    6        (pins stock order cost; epoch<6 的一切数字与本行不可比, measurementValid(5)=false)
# ranAt:            2026-09-20
#
# flags:
#   ⚠⚠ 零滑点高估 —— 满仓微盘个股 × 零滑点台。ABSOLUTE LEVELS ARE NOT REALIZABLE，
#      尤以 VAL 的 544.14% / annual 153.80% 为甚（本家族/本项目产出过的最被夸大的一组数字）。
#      只有同腿、同纪元臂间的 Δ 可信（etfmom-004 -> 005 是一行 diff）。
#   ⚠⚠ VAL 不是 TRAIN 的干净孪生 —— apply_nine_point_audit 由 `> 2025-01-01` 门控，
#      VAL 窗的 2025 那一半执行的是 TRAIN 零覆盖的代码路径。该 VAL 是合法的单文件单台测量，
#      但不是对 TRAIN 所测机制的纯样本外重放。
#   ⚠  VAL 跨 2024 年 1-2 月微盘踩踏，而 avoid_trade_april 每年 1/4 月轮进 511880 货币 ETF
#      => VAL 窗内新增 4 次触发，而该规则的 TRAIN 证据只有 n=4 且已标 overfit-suspect。
#      高 VAL 夏普可能部分来自这条规则躲开了 2024 回撤：待审查，不得读作持久技能。
#   ⚠  零新颖 —— 本配置与 study q-10 等价（见 wiki/families/ETF动量.md §2 / q-2 / q-8 / q-9 / q-10）。
#      记作「q-10 配置在 epoch 6 上的重测 + 首次 VAL」，不是一次 enhance 发现。
#   ⚠  vs 家族基线 etfmom-003 (0.2944) 的 Δ 是跨成员比较、不是消融（不同源文件、现实性等级不同）；
#      vs etfmom-004 (0.6459) 的 +0.0057 才是一行 diff 的干净读数。
#   ⚠  标签矛盾 —— 本臂实质是一本小市值书挂着 ETF动量 的家族名；家族页裁定「挂旗不裁」(精度 c374bf5)。
#   ⚠  2025/OOS 口径的独立检验在本流水线不存在，且 OOS 窗硬阻断（OOS-BLOCKED）。
#
# 详情附页: wiki/experiments/etfmom-005.md   账本: enhance/results.tsv   源码: enhance/candidates/etfmom-005.py
# ================================================================================
