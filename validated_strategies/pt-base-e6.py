# ===== VALIDATED STRATEGY (run-family archive) — join-quant epoch 6 =====
# expId:            pt-base-e6
# ideaId:           (none — the base itself; both improve candidates lost on TRAIN: pt-imp-1 −0.57, pt-imp-2 −0.14)
# baseExpId:        baseline-e6  (= base fa0d3bd9 + epoch-6 OVERRIDE, study/PT多策略/baseline-e6.py; reproduces the epoch-4 ledger row digit for digit)
# family:           PT多策略   (source base: strategies/2026-06-23_P-T多策略并行实战_用_quot_自有账本_quot_实现四大策略各安其位-fa0d3bd9.py)
# mutation:         none. Built by study/PT多策略/build-e6.js (py2to3(source) + live OVERRIDE).
# train_objective:  1.8592   (TRAIN 2022-01-01..2023-12-31: total 772.01% / annual 195.74% / maxDD 9.82% / sharpe 11.15; 2022 +300.50 / 2023 +121.38)
# val_objective:    2.3581   (VAL   2024-01-01..2025-12-31: total 1067.81% / annual 241.73% / maxDD 5.92% / sharpe 12.82; 2024 +373.60 / 2025 +147.40)
# sharpe_val:       12.82
# gate_val:         pass     (sharpe 12.82 >= 1.5)
# harness_epoch:    6
# ranAt:            2026-09-23
#
# flags:
#   ⚠⚠ NOT REALIZABLE, and measured so (study-u-7): the same signal executed at 14:50 instead of the 09:30 open
#      returns −29.49% / maxDD 39.96 on TRAIN — the same magnitude as the no-signal thinnest-10 control (u-4, −42%).
#      The whole return is the opening-auction print of thin ETFs (previous-day turnover 5e6–2e7) below T−1 NAV,
#      filled at 5% of the DAY's volume. Real auction volume on these names is a small fraction of that.
#   ⚠⚠ The 5% participation cap alone halved the epoch-2 number (annual 369 → 196): capacity, not signal.
#   ⚠  The title (四大策略并行 / 自有账本) describes nothing in the code; c70281d3's 分仓隔离插件 is dead code (q-struct).
#   Edges measured: 均值回归 (mirror −91%, control −42%, base +772%, log-symmetric) and 流动性溢价 (band monotone in
#   thinness: 5e6–2e7 196 / 1e7–2e7 138 / 5e7–1e8 23 annual). Recommended family status: DQ-realizability (human call).
#   Full write-up: wiki/families/PT多策略.md §1/§2/§4/§6.
# ======================================================================
