...
def get_stock_list(context): 
    ...
    if len(g.hl_counts) > 20:  # imp-3: sentiment gate
        __prior = sorted(g.hl_counts[:-1])
        if g.hl_counts[-1] < __prior[len(__prior) // 2]:
            return []
    qualified_stocks = []
    sbdk_stocks = []
    sbgk_stocks = []
...
def prepare_stock_list(context):
    ...
    g.hl_counts = (getattr(g, 'hl_counts', []) + [len(hl_list)])[-21:]  # imp-3: raw limit-up breadth
