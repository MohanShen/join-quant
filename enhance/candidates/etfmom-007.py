...
# enhance idea-3 (etfmom-007). Drop size-ranked candidates whose latest DISCLOSED
# shareholder count rose by more than HN_MAX_RISE against the prior disclosure (chip
# dispersion). It only removes names, so market cap stays the primary sort and the next
# size rank backfills. Reads strictly pub_date <= previous_date. Names with fewer than
# two disclosures are KEPT. Not wrapped in try/except on purpose: a data failure must
# surface as an error, not as a run that silently equals its control (etfmom-005).
HN_MAX_RISE = 0.10


def filter_holder_num(context, stock_list):
    if not stock_list:
        return stock_list
    ...
    df = finance.run_query(
        query(
            ...
            finance.STK_HOLDER_NUM.code.in_(stock_list),
            finance.STK_HOLDER_NUM.pub_date <= prev,
            finance.STK_HOLDER_NUM.pub_date > prev - timedelta(days=400),
        )
    )
    covered, dropped = 0, []
    if df is not None and len(df) > 0:
        df = df.dropna(subset=["share_holders"])
        df = df[df["share_holders"] > 0]
        for code, grp in df.groupby("code"):
            grp = grp.sort_values(["end_date", "pub_date"]).drop_duplicates("end_date", keep="last")
            if len(grp) < 2:
                continue
            covered += 1
            last, before = grp["share_holders"].iloc[-1], grp["share_holders"].iloc[-2]
            if last / before - 1 > HN_MAX_RISE:
                dropped.append(code)
    record(hn_candidates=len(stock_list), hn_covered=covered, hn_dropped=len(dropped))
    return [s for s in stock_list if s not in dropped]
...
def get_small_cap_stocks_v3(context):
    ...
    # ==== enhance idea-3 (etfmom-007): 股东户数 filter, size stays the primary sort ====
    candidate_list = filter_holder_num(context, candidate_list)
