...
# title: 低换手红利策略
# ---
# improve candidate i-4 of study base snapshot — family: 红利低频
...
def get_dividend_yield_ttm(stock_list, date):
    ...
    q = query(
        ...
        finance.STK_XR_XD.board_plan_pub_date,       # i-4: 董事会预案公告日
        finance.STK_XR_XD.plan_progress
    ).filter(
        finance.STK_XR_XD.code.in_(stock_list),
        finance.STK_XR_XD.board_plan_pub_date >= str(start_date),  # i-4: by announcement date
        finance.STK_XR_XD.board_plan_pub_date <= str(date)  # i-4: no plan_progress filter (would use ex-post status)
    )
