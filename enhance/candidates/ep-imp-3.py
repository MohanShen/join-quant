...
def initialize(context):
    ...
    run_daily(market_open, '14:50', reference_security='000300.XSHG')  # ep-imp-3: signal + fill at 14:50

def after_market_open(context):
    # 获取基金
    fund_list = get_all_securities(['etf'], context.previous_date).index.tolist()  # u-2: ETF only
    # 成交额过滤
...
def market_open(context):
    ...
    ## 获得基金最新价
    __px = get_price(df.index.tolist(), end_date=context.current_dt, frequency='1m', fields=['close'], count=1, panel=False)  # ep-imp-3: 14:49 minute close
    __px = __px.set_index('code')['close'] if 'code' in __px.columns else __px['close']
    df['last_price'] = [float(__px.get(c, float('nan'))) for c in df.index.tolist()]
    ## 计算溢价
