...
# expId:    jul12-002  (idea-2: +风控·止损线 -8%)
# base:     jul12-000 (baseline objective(TRAIN)≈1.3738 / sharpe≈5.81)
# mutation: 在 14:50 的 stop_profit 里追加独立的 -8% 个股止损(跌停不卖); 保留破开盘价清仓/止盈+5%不变.
#           不动开仓逻辑与 1/4月空仓. 只此一个受控因子(风控·止损线).
...
def stop_profit(context):
    """
    14:50 统一处理: (1) 止盈+5%(未涨停); (2) idea-2 新增独立 -8% 止损(跌停不卖).
    两条规则互相独立, 不改动止盈原逻辑.
    """
    data_today = get_current_data()
    for stock, pos in context.portfolio.positions.items():
        ...
        # idea-2: -8% 止损 —— 亏损达阈值且非跌停(跌停不卖)则清仓
        if profit_pct <= -8 and curr.last_price != curr.low_limit:
            close_position(stock)
            continue

        # 止盈+5% —— 涨停则保留(可能继续冲高), 否则达标止盈
