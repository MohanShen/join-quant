...
# ===== AUTHOR-COST PROBE (bypasses the OVERRIDE shadow) =====
# Applies the strategy AUTHOR's own declared stock costs (line 408: close_tax 0.0005,
# commission 0.85/10000) via the captured original, i.e. what epoch 5 effectively ran,
# since the epoch-5 shadow forwarded type=stock through unchanged.
# Control = etfmom-004.py (bench pin) at 215.05/77.64/3.10/13.05.
#   different -> 215.05 is the true bench-pinned epoch-6 value; the arm is trustworthy.
#   identical -> author and bench stock costs give the same result, which contradicts the
#                measured 1%/side sensitivity and means a run is being misread.
