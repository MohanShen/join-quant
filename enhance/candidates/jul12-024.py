...
# expId:    jul12-024  (idea-21 Arc-4 peak-locate = jul12-023 + 对冲比 1.2x -> 1.3x)
# baseExpId: jul12-023  (市场中性 微盘多头 − IC 空头 1.2×; TRAIN obj 0.3202, sharpe 2.64 PASS, maxdd 12.27%)
...
#           -> 最小 10 只等权, run_weekly 调仓. 空头对冲(子仓1): 做空 IC 主力合约, 名义规模≈1.3×多头
#           市值 (探 min-variance 峰值), 主力换月时滚动. Factors §2.1: 选股·规模价值[小市值];
...
def rebalance(context):
    ...
    if price is None or price <= 0 or long_value <= 0:
        ...
        contracts = int(round(1.3 * long_value / notional_per))   # 对冲比 1.3×
