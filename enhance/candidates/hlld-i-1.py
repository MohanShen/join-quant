...
# title: 低换手红利策略
# ---
# improve candidate i-1 of study base snapshot — family: 红利低频
# base archetype = 93379f98 (dividend-yield TTM rank + 20d low-turnover rank, 10 names, 25-day cycle).
# Frozen harness cost OVERRIDE appended below (harness.md §2), from strategy-normalize.js.
...
def get_dividend_yield_3y(stock_list, date):
    """i-1 (borrowed from 2596a8ab): average annual cash dividend over the last 3 years / market cap.
    bonus_amount_rmb is in 万元, market_cap in 亿元. Uses implemented plans by registration date, as the base does."""
    ...
    if not df_all:
        return pd.Series(dtype=float)
    df_div = pd.concat(df_all, ignore_index=True)
    df_div = df_div[df_div['bonus_amount_rmb'].notna() & (df_div['bonus_amount_rmb'] > 0)]
    if len(df_div) == 0:
        return pd.Series(dtype=float)
    div_avg = df_div.groupby('code')['bonus_amount_rmb'].sum() / 3.0
    cap = get_fundamentals(query(valuation.code, valuation.market_cap).filter(
        valuation.code.in_(list(div_avg.index))), date=date).set_index('code')['market_cap']
    dy = (div_avg / 1e4) / cap
    return dy.dropna()
...
def calc_factor_rank(context, stock_list):
    ...
    # --- 获取股息率TTM ---
    dy_series = get_dividend_yield_3y(stock_list, prev_date)  # i-1: 3y avg cash dividend / market cap
