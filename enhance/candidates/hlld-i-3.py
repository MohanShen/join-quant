...
# title: 低换手红利策略
# ---
# improve candidate i-3 of study base snapshot — family: 红利低频
...
def calc_factor_rank(context, stock_list):
    ...
    # i-3 (borrowed from 1a8a41cf): earnings-quality screen against value traps
    qf = get_fundamentals(query(valuation.code).filter(
        valuation.code.in_(stock_list),
        valuation.pe_ratio.between(5, 50),
        indicator.inc_return > 5,
        indicator.inc_net_profit_year_on_year > 0,
    ), date=prev_date)
    stock_list = list(qf['code'])
