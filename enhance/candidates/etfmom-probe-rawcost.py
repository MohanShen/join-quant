...
# ===== RAW STOCK-COST PROBE (bypasses the OVERRIDE shadow) =====
# The OVERRIDE rebinds set_order_cost to a shadow that DISCARDS the caller argument and
# re-applies the bench values. So a probe calling set_order_cost() re-applies the pin
# instead of injecting a test cost. This calls the captured ORIGINAL directly.
# Deliberately NOT wrapped in try/except: a NameError here is itself the answer, because
# it would mean the OVERRIDE shadow block never installed (set_order_cost unbound at
# module scope) and therefore the epoch-6 stock pin never applied at all.
__probe_orig_initialize = initialize
def initialize(context):
    __probe_orig_initialize(context)
    __jq_set_order_cost(
        OrderCost(open_commission=0.01, close_commission=0.01,
                  close_tax=0.001, min_commission=5),
        type="stock",
    )
# ===== END RAW STOCK-COST PROBE =====
