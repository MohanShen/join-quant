...
# expId:    jul12-001  (idea-1: +择时·趋势均线 大盘趋势买入闸门)
# base:     jul12-000 (baseline objective(TRAIN)≈1.3738 / sharpe≈5.81)
# mutation: 在 buy() 加入 国证2000(399303.XSHE) MA20 买入闸门——仅当最新已完成日线收盘 >= MA20 才允许新开仓;
#           不动卖出(破开盘价清仓/止盈+5%)与 1/4月空仓日历. 只此一个受控因子.
# harness:  research/harness.md (冻结成本 OVERRIDE 见文末; 迭代只跑 --window train)
# note:     ⚠ 低开剥头皮=高换手, 零滑点下 objective 偏高估 (harness §2).
...
def buy(context):
    ...
    # ===== idea-1: 大盘趋势买入闸门 (择时·趋势均线) =====
    # 仅当 国证2000 最新已完成日线收盘 >= MA20 才允许新开仓; 否则今日不开新仓(不影响卖出).
    # attribute_history 只返回已完成K线, 无未来数据泄漏.
    try:
        idx_hist = attribute_history('399303.XSHE', 20, '1d', ('close',),
                                     skip_paused=False, df=False, fq='pre')
        idx_close = idx_hist['close']
        if len(idx_close) >= 20:
            idx_ma20 = idx_close.mean()
            if idx_close[-1] < idx_ma20:
                return  # 大盘趋势向下, 今日不开新仓
    except Exception:
        pass  # 取数失败则退化为不设闸门(与 baseline 一致)
    # ===== end idea-1 gate =====
