...
# expId:    jul12-014  (idea-11 Arc-2 first mutation = jul12-013 + 国证2000 MA20 大盘防御空仓)
# baseExpId: jul12-013  (min-cap 周度轮动 baseline; TRAIN score 0.2059, sharpe 1.65 DQ)
# harness:  research/harness.md (冻结成本 OVERRIDE 见文末; 迭代只跑 --window train)
# design:   min-cap 周度轮动 + 择时·趋势均线防御: 调仓前若 399303 收盘 < MA20 则清仓空仓本轮不买.
#           universe_size=400 -> hold_count=10 等权; run_weekly(weekday=1, open).
# filters:  §3 强制真实性 — 剔除 ST/*ST/退, 停牌, 次新(<250自然日), 涨停开盘不买/跌停不卖;
#           另剔除 688* (科创板) 与 30* (创业板 300/301).
# note:     Arc-2 目标: 过 sharpe>=2.5 门槛 (baseline DQ 1.65). 观察 maxdd 是否由 25.4% 下降且 annual 保住.
...
def rebalance(context):
    # 择时·趋势均线：国证2000 收盘 < MA20 -> 大盘防御，清仓空仓，本轮不买
    h = attribute_history('399303.XSHE', 20, '1d', ['close'])
    ma20 = h['close'].mean()
    cur = h['close'][-1]
    if cur < ma20:
