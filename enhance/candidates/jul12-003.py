...
# expId:    jul12-003  (idea-3 = jul12-000 + 国九条 质量筛 in get_stocks)
# baseExpId: jul12-000
...
# mutation: get_stocks() 的 fundamentals query 加 .filter(income.net_profit>0, income.operating_revenue>1e8)
#           —— 在取市值最小 g.choice(=1000) 之前先过 国九条 盈利/营收质量门槛; 其余全部与 baseline 位同.
# note:     baseline TRAIN objective≈1.3738 / sharpe≈5.81. 单因子小步变异.
...
def get_stocks(context):
    """
    从A股中按市值从小到大选取前 g.choice 只股票（先过 国九条 盈利/营收质量筛），再过滤掉特殊股票
    """
    # 查询：股票代码 + 市值，先按 国九条 质量筛（净利润>0 且 营收>1亿），再按市值升序取前 g.choice 只
    q = query(valuation.code, valuation.market_cap).filter(income.net_profit > 0, income.operating_revenue > 1e8).order_by(valuation.market_cap.asc()).limit(g.choice)
