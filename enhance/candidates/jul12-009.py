...
# expId:    jul12-009  (idea-6 = jul12-005 + gap子阈值 chg<-1 -> chg<-1.5)
...
# mutation: 选股·技术量价 单因子: is_eligible_for_buy 开盘跳空阈值 chg<-1% -> chg<-1.5% (更深的低开).
# note:     bar to beat obj(TRAIN) > 1.3898. 收缩候选池 -> runtime-safe. 若日内不足6只可能现金拖累(年化下滑).
...
def is_eligible_for_buy(context, stock, data_today, history):
    ...
    if not (today_open < prev_low and chg < -1.5):
        return False
