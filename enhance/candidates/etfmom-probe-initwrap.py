...
# ===== INITIALIZE-WRAPPER REACHABILITY PROBE =====
# The frozen OVERRIDE applies epoch 4/5/6 pins (order_volume_ratio, avoid_future_data,
# fund costs, stock costs) by REBINDING initialize. Module-level code is already proven
# to run. This asks the next question: does JQ actually call OUR rebound initialize?
#   run FAILS     -> wrapper IS called; the inert pin needs another explanation.
#   run COMPLETES -> wrapper is NEVER called; every pin applied this way is inert.
try:
    __probe_orig_init = initialize
    def initialize(context):
        __probe_orig_init(context)
        raise Exception("PROBE-INIT-WRAPPER-CALLED")
except NameError:
    pass
# ===== END INITIALIZE-WRAPPER PROBE =====
