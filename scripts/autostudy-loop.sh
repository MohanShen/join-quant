...
NUDGE="Quota is available again — continue the auto-study batch exactly where you left off (you are already running /run-study per study/program.md, FAMILY-LEVEL). Work study/manifest.json in order: finish the in-progress family, mark it done, take the next pending family (strongest bestObjective first); do NOT exit until all are done or I say stop. Have the experimenter run the backtester plain with --usage-limit $USAGE_LIMIT (no JQ_USAGE_LIMIT= prefix, no | tail) and IN THE FOREGROUND (blocking) — never run_in_background + await a notification (it won't re-invoke this headless session). Write findings back to each family page §2/§6 and refresh §3 via node utils/wiki-family-build.js. NEVER touch the reserved OOS window (epoch 5: 2026-01-01+; confirm with `node utils/harness-config.js` — the boundary moves with the epoch, and 2025 is now VAL). Keep going until the JQ budget (used>=$USAGE_LIMIT) or the Anthropic quota is hit, then STOP at a clean git state with a one-line status (which family, how far). Do NOT git commit wiki/ledgers unless asked."
