#!/bin/bash
#
# autoenhance-interactive.sh — start (or reopen) the autoenhance loop in an INTERACTIVE
# claude session with a PINNED session id, so the hourly cron (scripts/autoenhance-loop.sh)
...
#
# The pinned id is stored in data/autoenhance-session.txt as "<branch>\t<uuid>" (gitignored).
...
if [ -n "$SID" ] && ls "$HOME"/.claude/projects/*/"$SID".jsonl >/dev/null 2>&1; then
  echo "Reopening autoenhance session $SID on $BRANCH"
  ...
  printf '%s\t%s\n' "$BRANCH" "$SID" > "$SID_FILE"
  echo "New autoenhance session $SID on $BRANCH"
  echo "→ Inside claude, type:  /run-enhance   (to begin the epoch)"
