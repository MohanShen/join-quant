...
> 记账定位：这是 **KB-only 负结果**（Arc-2 无任何过门槛/定稿版本），故**不占 `enhance/results.tsv` 行、不归档 `validated_strategies/`**。此页把「此方向不成立」的知识固化，避免被重推。⚠ **零滑点高估**：本轮动族仍高换手（周度全书轮动，虽比日内剥头皮温和），零滑点下 objective 偏高、换手成本被忽略。
