...
| ETF溢价 | ETF 折价均值回归（illiquidity premium） | 基金溢价, 溢价回撤 |
| PT多策略 | 多策略并行 / 分仓隔离框架 | P-T, 自有账本, 分仓 |
| 三进兵 | 三条 EMA（5/20/60）组合的个股择时 overlay：EMA5 上穿 EMA60 且 EMA20 在下时买、跌破 EMA20 止损 | 三进兵, 三EMA均线组合 |
| 大小盘轮动 | 大小盘 / 白马 风格轮动 | 风格轮动, 白马 |
| 其他 | 单例桶：未成血统的一次性策略，**不建家族页** | one-off |
...
| 流动性溢价 | risk-premium | edge pass 2026-09-21（[[ETF溢价]] q-1 / [[PT多策略]] q-1） | illiquidity premium、薄基金折价 |
...
> [[大小盘轮动]]）的收益都由**规模因子**供给，其中三个的家族名与之无关（ETF动量、多因子ML、七星高照
> 的过闸成员都是「某腿 + 小市值腿」的混合，而小市值腿是唯一供给收益的那条）。这正是
> `component-scan` 在 epoch 6 上测到「每个 type 的每个候选 uplift 都为负」的原因——收益相关性把它们
> 看成不同的东西，共享 edge 不会。
