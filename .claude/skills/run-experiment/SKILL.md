...
## 团队（角色定义见 `.claude/agents/autoresearch-*.md`）
...
- **Agent 4 `autoresearch-recorder`** —— 仅 VAL 结果触发：记 `results.tsv`+实验页+回填 KB，交回 Agent 1。
...
- 在实验分支 `research/<tag>` 上。**断点续跑**：先读 `research/loop-state.json`（上次停在哪个 agent/阶段）+ `research/ideas-queue.json`（已测/待测想法）+ `research/results.tsv`（已定稿），据 `phase`/`nextAction` 从断点继续，不重跑已完成回测、不重复已测想法。三者都不存在 → 按 `program.md` Setup 初始化（全新纪元）。

## 运行方式（前台交互，可观察）
作为编排者，按 `program.md` 状态机**依次调度**四个 agent（用 Agent 工具按名字生成对应角色，把上一环节的产物/结论作为输入传入），在它们之间路由：
`ideator →(想法) critic →(出队) engineer →(train) ideator →(定稿) engineer →(val) recorder → ideator …`。
每一步都在前台可见；用户可随时打断或说「停」。**只有用户说停才结束**（预算到顶=干净暂停，非停止）。
