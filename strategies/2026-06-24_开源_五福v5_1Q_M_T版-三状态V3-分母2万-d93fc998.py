...
# 克隆自聚宽文章：https://www.joinquant.com/post/72400
# 标题：五福51-三状态V3-分母2万-11年711倍回撤19.74
# 作者：rbq2025
...
#行业ETF：
    ...
    g.afternoon_sell_time = '13:03'
    g.afternoon_buy_time = '13:05'
...
def log_pending_short_momentum_3up_followups(context):
    ...
    for item in pending:
        ...
        log.info(
            ...
            f"  放强追弱风险标签: {risk}  |  短斜率={slp_s_line}  长斜率={slp_l_line}  |  依���: {risk_reason}\n"
            f"{'=' * 72}"
        )
...
# #coding=gbk
# """
# #五福闹新春v5.1 · QMT版本 · 三状态自适应动量轮动
# #原聚宽策略：https://www.joinquant.com/post/72254
# 原作者：烟花三月、rbq2025
# 聚宽转qmt作者：Ai夸我像真人
# 核心逻辑：
#   1. 根据6个宽基指数与MA10/MA20的关系，将市场划分为「正常期/震荡期/走弱期」
#   2. 不同状态下启用不同的过滤条件（高斯滤波仅在震荡期启用等）
#   3. 计算长期(25日)和短期(21日)加权对数回归动量得分
#   4. 五档即时成交，含止损和组合回撤控制
# """
...
#         wk = (wecom_webhook_key or '').strip()
#         self._wecom_webhook_key: Optional[str] = wk or None
...
#             print(f'[init][回测模式][初始资金:{self.strategy_initial_capital}][回测周期:{context_info.start}~{context_info.end}][{context_info.period}]')
#         else:
...
#         print(f'[{self.stock_code}][{self.acct_id}][{self.acct_type}] 策略初始资金:{self.strategy_initial_capital}, 回测={self.is_backtesting}, {context_info.period}')
...
#         return float(self.get_broker_available_cash())

#     def get_strategy_total_value(self, context_info) -> float:
#         total_cash = self.get_strategy_available_cash()
...
#             holdinglist[stock_key] = _holding_snapshot_from_counter_row(dt)
#             if print_holdings:
...
#                 if info: cache[symbol] = info
#             info = cache[symbol]
#             if info:
#                 if field: return info.get(field)
#                 return info
#             return None
#         except Exception:
#             return None
...
#         if (not open_date and info.get('PreClose') == 1.0) or open_date == 19700101: return False
#         if date == '': date = int(datetime.now().strftime("%Y%m%d"))
#         else: date = int(date)
#         if date < open_date: return False
#         expire_date = info.get('ExpireDate', 0)
#         if expire_date and date >= expire_date: return False
#         return True

#     def get_valid_stock_list(self, sectors: List[str] = None, date = '') -> List[str]:
#         if sectors is None: sectors = ['沪深ETF']
...
#                     if not self.is_backtesting and info.get('InstrumentStatus', 0) >= 1: continue
#                     if self.is_valid_stock_in_date(code, date): valid_codes.append(code)
#                 except:
#                     continue
#             return valid_codes
#         except Exception as e:
...
#             if tick_data: return tick_data['lastPrice']
#             return None
...
#         if not end_date_inclusive: count += 1
#         raw_dates = self.contextInfo.get_trading_dates('SH', start_date, end_date, count, '1d')
#         if raw_dates:
#             trading_days: List[str] = []
#             for d in raw_dates:
#                 if end_date_inclusive or d != end_date: trading_days.append(d)
#             return trading_days
#         return []
...
#                 )
#                 _cache_raw_data(new_data, fetch_fields)
#             except Exception:
#                 pass
#         result: Dict[str, Dict[str, pd.Series]] = {}
#         if not fields: fields = self._daily_fields_preset
#         for code in codes:
#             entry = cache.get(code)
...
#     g['dl_history_data'] = True
#     g['enable_detailed_log'] = True
#     g['dynamic_etf_pool_enable'] = True
#     g['filtered_fixed_pool'] = []
#     g['filtered_global_pool'] = []
#     g['dynamic_etf_pool'] = []
#     g['merged_etf_pool'] = []
#     g['ranked_etfs_result'] = []
#     g['full_ranked_etfs_result'] = []
#     g['filtered_ranked_etfs_result'] = []
#     g['target_etfs_list'] = []
#     g['yesterday_close_cache'] = {}
#     g['last_metrics_by_etf_code'] = {}
...
#     g['laplace_min_slope'] = 0.002
#     g['enable_laplace_s_regime_differentiation'] = True
#     g['laplace_s_param_normal'] = 0.06
#     g['laplace_s_param_weak'] = 0.12

#     # === 高斯滤波（震荡期专用） ===
#     g['gaussian_sigma'] = 1.2
#     g['gaussian_min_slope'] = 0.002
#     g['gaussian_use_relative_slope'] = True
#     g['gaussian_min_slope_relative'] = 0.0013
...
#     g['weak_apply_volume_filter'] = True
#     g['weak_apply_loss_filter'] = True
#     g['weak_apply_premium_filter'] = False
...
#     # === 定时任务标记 ===
#     g['morning_routine_done'] = False
#     g['momentum_calculated'] = False
#     g['afternoon_sell_done'] = False
#     g['afternoon_buy_done'] = False
#     g['reset_daily_done'] = False
#     g['history_last_dl_date'] = None
...
#         '520830.SH','159529.SZ','164824.SZ','513850.SH','513080.SH',
#         '513730.SH','511380.SH','511010.SH','511220.SH',
#     ]
#     g['china_etf_pool'] = [
#         '513090.SH','513120.SH','513180.SH','513330.SH','513750.SH',
#         '159892.SZ','513190.SH','159605.SZ','513630.SH','159323.SZ',
#         '510900.SH','513920.SH','513970.SH','511380.SH','512050.SH',
#         '510500.SH','159915.SZ','510300.SH','512100.SH','159949.SZ',
#         '588080.SH','159967.SZ','588220.SH','563300.SH','510760.SH',
#         '588200.SH','515880.SH','159981.SZ','512880.SH','513350.SH',
#         '159326.SZ','159516.SZ','159206.SZ','512480.SH','159363.SZ',
#         '159870.SZ','512400.SH','159755.SZ','588170.SH','159992.SZ',
#         '159995.SZ','512890.SH','515220.SH','159566.SZ','159819.SZ',
#         '512800.SH','512690.SH','515050.SH','562500.SH','512170.SH',
#         '517520.SH','159869.SZ','512070.SH','159611.SZ','562800.SH',
#         '515120.SH','512010.SH','510880.SH','515790.SH','515980.SH',
#         '512660.SH','159928.SZ','512710.SH','560860.SH','515030.SH',
#         '159766.SZ','159218.SZ','159852.SZ','516160.SH','516150.SH',
#         '159227.SZ','159583.SZ','588790.SH','159865.SZ','512980.SH',
#         '159851.SZ','561360.SH','561980.SH','562590.SH','512200.SH',
#         '159732.SZ','159667.SZ','516510.SH','159840.SZ','159998.SZ',
#         '159825.SZ','512670.SH','159883.SZ','515210.SH','515400.SH',
#         '159256.SZ','561330.SH','515170.SH','159638.SZ','516520.SH',
#         '513360.SH','516190.SH',
#     ]
#     g['fixed_etf_pool'] = g['global_etf_pool'] + g['china_etf_pool']
...
#     print_init_params()
#     for code in g['fixed_etf_pool']: ContextInfo.subscribe_quote(code, period='1m')

# def print_init_params():
#     print(f"""#【五福闹新春】v5.1 · QMT · 三状态自适应轮动 —— 卖出13:10 / 买入13:11，五档即时成交
# #【策略参数初始化完成】
# # === ETF池 ===
# # - 全球/海外ETF池: {len(g['global_etf_pool'])}只
# # - 国内ETF池: {len(g['china_etf_pool'])}只
# # - 固定池合计: {len(g['fixed_etf_pool'])}只
# # === 市场分型 ===
# # - 状态: 正常期/震荡期/走弱期，确认天数={g['regime_switch_confirm_days']}
# # - 指数: 沪深300/深证综指/创业板/中证A500/中证1000/国证2000
# # - 进入走弱期: ≥{g['regime_weak_below_ma20_min']}个指数低于MA{g['regime_ma20_lookback']}
# # - 进入正常期: ≥{g['regime_normal_above_ma10_min']}个指数站上MA{g['normal_ma_lookback']}
# # === 动量计算 ===
# # - 长期={g['lookback_days']}日, 短期={g['short_momentum_lookback']}日
# # === 过滤条件 ===
# # - R2: 启用(阈值>{g['r2_threshold']}), 震荡期额外R2否决={g['enable_range_r2_veto']}
# # - 拉普拉斯: 启用(s={g['laplace_s_param']}, 分状态差异化={g['enable_laplace_s_regime_differentiation']})
# # - 高斯滤波: 震荡期专用
# # - 长短动量双正: 震荡期专用
# # === 交易 ===
# # - 13:10 卖出 / 13:11 买入
# # - 止损: 成本价×{g['fixedStopLossThreshold']:.0%}
# # === 防频换 ===
# # - 正常期容忍={g['normal_max_days_not_rank1']}日, 震荡期容忍={g['oscillation_max_days_not_topk']}日
# """)
...
#     global trader
#     curdatetime = trader.get_current_time(ContextInfo, '%Y%m%d%H%M')
#     cur_date = curdatetime[:8]
#     cur_time = curdatetime[8:]

#     is_last_bar = ContextInfo.is_last_bar()
#     is_backtest = ContextInfo.do_back_test

#     if not is_backtest and not is_last_bar:
#         return
...
#         g['normal_streak_hold_code'] = None
#         g['normal_not_topk_streaks'] = {}
#         print(f'★ [{cur_date}] {cur_time} 新自然日开始 ★')
#         download_etf_his_data(cur_date, ContextInfo)

#     # 09:00 晨间流水线
#     if not g['morning_routine_done'] and cur_time >= '0900':
#         morning_routine(ContextInfo, cur_date, cur_time)
#         g['morning_routine_done'] = True

#     # 13:10 卖出（先判定市场状态，再计算动量，再卖出）
#     if not g['afternoon_sell_done'] and cur_time >= '1310':
#         resolve_market_regime(ContextInfo, cur_date)
#         refresh_holdings_num_by_regime()
#         ensure_merged_pool_available(cur_date)
#         if not g['momentum_calculated']:
#             print("【动量计算】开始...")
#             calculate_and_log_ranked_etfs(ContextInfo, cur_date, cur_time)
#             g['momentum_calculated'] = True
#         print("【卖出执行】开始...")
#         execute_sell_trades(ContextInfo)
#         g['afternoon_sell_done'] = True

#     # 13:11 买入
#     if not g['afternoon_buy_done'] and cur_time >= '1311':
#         if not g['momentum_calculated']:
#             ensure_merged_pool_available(cur_date)
#             resolve_market_regime(ContextInfo, cur_date)
#             refresh_holdings_num_by_regime()
...
#         g['afternoon_buy_done'] = True

#     minute_level_stop_loss(ContextInfo, cur_time)
#     minute_level_pct_stop_loss(ContextInfo, cur_date, cur_time)

#     if not g['reset_daily_done'] and cur_time >= '1510':
#         reset_daily_flags(ContextInfo, cur_date)
#         g['reset_daily_done'] = True

# # ============================================================================
# # 【市场状态分型】三状态：正常期 / 震荡期 / 走弱期
# # ============================================================================
# def resolve_market_regime(ContextInfo, cur_date: str):
#     indexes = g['regime_indexes']
#     n_index = len(indexes)
#     bars_need = max(g['regime_ma20_lookback'], g['normal_ma_lookback']) + 1
#     end_day = trader.get_prev_trade_date(cur_date)
#     below_ma20 = 0
#     above_ma10 = 0
#     n_ok = 0

#     for name, code in indexes.items():
#         market_data = trader.get_daily_data_cached(['close'], [code], count=bars_need, end_time=end_day)
#         if not market_data or code not in market_data:
#             continue
#         data = market_data[code]
#         if 'close' not in data or len(data['close']) < g['regime_ma20_lookback']:
#             continue
#         closes = data['close'].values
#         cur = closes[-1]
#         ma10 = np.mean(closes[-g['normal_ma_lookback']:])
#         ma20 = np.mean(closes[-g['regime_ma20_lookback']:])
#         if cur < ma20:
#             below_ma20 += 1
#         if cur > ma10:
#             above_ma10 += 1
...
#     print(f"【市场状态】below_ma20={below_ma20}/{n_ok}(走弱阈值≥{weak_min}), above_ma10={above_ma10}/{n_ok}(正常阈值≥{normal_min}) → 指标【{raw_regime}】")
...
#     regime_show = {'正常期':'??正常期', '震荡期':'??震荡期', '走弱期':'??走弱期'}.get(g['market_regime'], g['market_regime'])
#     print(f"【市场状态·生效】→ {regime_show}")

# def refresh_holdings_num_by_regime():
#     regime = g['market_regime']
#     target_map = {'正常期': g['holdings_num_normal'], '震荡期': g['holdings_num_oscillation'], '走弱期': g['holdings_num_weak']}
#     g['holdings_num'] = max(1, target_map.get(regime, g['holdings_num']))

# def ensure_merged_pool_available(cur_date: str):
#     if g['market_regime'] == '走弱期':
#         if 'filtered_global_pool' in g and g['filtered_global_pool']:
#             g['merged_etf_pool'] = list(set(g['filtered_global_pool']))
#         else:
#             g['merged_etf_pool'] = list(set(g['global_etf_pool']))
#         g['merged_etf_pool'].sort()
#         print(f"[RED] 【走弱期】使用过滤后全球/海外ETF池，共{len(g['merged_etf_pool'])}只")
#     else:
#         if not g.get('merged_etf_pool'):
#             print("[WARNING] merged_etf_pool 为空，立即重新合并")
#             daily_merge_etf_pools(ContextInfo)

# # ============================================================================
# # 【三】晨间流水线
# # ============================================================================
# def morning_routine(ContextInfo, cur_date, cur_time):
#     print("★" * 80)
#     print(f"【晨间流水线】启动... [{cur_date} {cur_time}]")
#     check_positions(ContextInfo)
#     monitor_drawdown(ContextInfo, cur_date)
#     calculate_global_etf_threshold(ContextInfo, cur_date)
#     # 早盘池更新
#     if g['market_regime'] == '走弱期':
#         filter_global_pool_by_volume(ContextInfo, cur_date)
#     else:
#         update_sector_pool(ContextInfo, cur_date)
#         filter_fixed_pool_by_volume(ContextInfo, cur_date)
#         daily_merge_etf_pools(ContextInfo)
#     print("【晨间流水线】执行完毕！")

# def reset_daily_flags(ContextInfo, cur_date: str):
#     g['yesterday_close_cache'] = {}
#     print("收盘缓存重置完成")
...
#     try:
#         holding_dict = trader.get_holdings()
#         for security, position in holding_dict.items():
#             if position.m_nVolume > 0:
#                 stock_name = get_stock_name(security)
#                 print(f"[DATA] 【持仓检查】{security} {stock_name}, 数量: {position.m_nVolume}, 成本: {position.m_dOpenPrice:.3f}")
#     except Exception as e:
#         print(f"【持仓检查】获取持仓失败: {e}")

# def monitor_drawdown(ContextInfo, cur_date):
#     try:
#         current_value = trader.get_strategy_total_value(ContextInfo)
#         if current_value > g['max_portfolio_value']:
#             g['max_portfolio_value'] = current_value
#         if g['max_portfolio_value'] > 0:
#             current_drawdown = (g['max_portfolio_value'] - current_value) / g['max_portfolio_value']
#             if current_drawdown >= g['drawdown_threshold']:
#                 record = {'date': cur_date, 'drawdown': current_drawdown,
#                           'portfolio_value': current_value, 'max_value': g['max_portfolio_value'],
#                           'market_regime': g['market_regime']}
#                 g['drawdown_records'].append(record)
#                 print(f"【回撤预警】回撤达到 {current_drawdown:.2%}")
#     except Exception as e:
#         print(f"【回撤监控】计算异常: {e}")

# # ============================================================================
# # 【五】流动性阈值 & 池过滤（与原有QMT代码相同）
# # ============================================================================
# def calculate_global_etf_threshold(ContextInfo, cur_date: str):
#     print("【全局阈值更新】")
#     try:
#         if not g['dynamic_etf_pool_enable']:
#             g['avg_etf_money_threshold'] = 10000000
#             return
#         etf_list = trader.get_valid_stock_list(['沪深ETF'], cur_date)
#         if not etf_list:
#             g['avg_etf_money_threshold'] = 10000000
#             return
#         end_date = trader.get_prev_trade_date(cur_date)
#         trade_days = trader.get_recent_trading_days(end_date=cur_date, count=3)
#         if len(trade_days) >= 3:
#             market_data = trader.get_daily_data_cached(['amount'], etf_list, count=3, end_time=end_date)
#             if market_data:
#                 cols = {}
#                 for stock, data in market_data.items():
#                     if 'amount' in data and len(data['amount']) > 0:
#                         cols[stock] = data['amount']
#                 if cols:
#                     amount_df = pd.DataFrame(cols).sort_index()
#                     daily_totals = amount_df.sum(axis=1)
#                     if len(daily_totals) >= 3:
#                         avg_total_money = daily_totals.mean()
#                         div = g['global_liquidity_threshold_divisor']
#                         threshold = avg_total_money / div
#                         g['avg_etf_money_threshold'] = threshold
#                         print(f" 阈值={threshold/1e4:.0f}万元")
#                         return
#         g['avg_etf_money_threshold'] = 10000000
#     except Exception as e:
#         print(f" [error]计算全局阈值异常: {e}")
#         g['avg_etf_money_threshold'] = 10000000

# def filter_global_pool_by_volume(ContextInfo, cur_date):
#     if g.get('avg_etf_money_threshold') is None:
#         calculate_global_etf_threshold(ContextInfo, cur_date)
#     global_pool = g['global_etf_pool']
#     if not global_pool:
#         g['filtered_global_pool'] = []
#         return
#     dynamic_threshold = g['avg_etf_money_threshold']
#     try:
#         market_data = trader.get_daily_data_cached(['amount'], global_pool, count=3, end_time=cur_date)
#         avg_daily_money = {}
#         for code in global_pool:
#             if code in market_data and 'amount' in market_data[code]:
#                 avg_daily_money[code] = market_data[code]['amount'].mean()
#         qualified = {k: v for k, v in avg_daily_money.items() if v > dynamic_threshold}
#         g['filtered_global_pool'] = list(qualified.keys())
#         print(f" 【全球池过滤】保留{len(g['filtered_global_pool'])}只")
#     except Exception as e:
#         print(f" 【全球池过滤】异常: {e}")
#         g['filtered_global_pool'] = global_pool[:]

# def update_sector_pool(ContextInfo, cur_date: str):
#     print("【动态池更新】")
#     if g['avg_etf_money_threshold'] is None:
#         calculate_global_etf_threshold(ContextInfo, cur_date)
...
#     SPECIAL_GROUPS = sorted([
#         {'name': '香港组', 'keywords': ['恒生', '恒指', '港股', '港股通', 'H股', '香港', '港', 'HKC', 'HK', 'HGS', 'H', '中概', 'HS科技'],
#          'remove_words': ['恒生', '恒指', '港股', '港股通', 'H股', '香港', '港', 'HKC', 'HK', 'HGS', 'H', '中概', 'HS']},
#         {'name': '科创组', 'keywords': ['科创', '科创板', '科综', 'KC', 'K C', '双创', '科创创业', '创创'],
#          'remove_words': ['科创', '科创板', '科综', 'KC', 'K C', '双创', '科创创业', '创创', '债券', '债汇', '债指', '债沪', '债易', '债基', '债兴', '债摩', '债', 'AAA']},
#         {'name': '创业组', 'keywords': ['创业板', '创业', '创板', '创成长'],
#          'remove_words': ['创业板', '创业', '创板', '创成长']},
#         {'name': '美指组', 'keywords': ['标普', '纳指', '纳斯达克'],
...
#     dynamic_threshold = g['avg_etf_money_threshold']
#     def filter_by_liquidity(etf_codes):
#         if not etf_codes:
#             return {}, []
#         try:
#             market_data = trader.get_daily_data_cached(['amount'], etf_codes, count=3)
#             avg_daily_money = {}
#             for code in etf_codes:
#                 if code in market_data and 'amount' in market_data[code]:
#                     avg_daily_money[code] = market_data[code]['amount'].mean()
#             qualified = {k: v for k, v in avg_daily_money.items() if v > dynamic_threshold}
#             removed = [c for c in etf_codes if c not in qualified]
...
#     special_groups = {}
#     for code in special_qualified:
#         try:
#             original_name = get_stock_name(code)
#             matched_group = special_group_map.get(code, '未知')
#             money = special_qualified[code]
#             cleaned = clean_name(original_name, is_special=True, matched_group_name=matched_group)
#             if cleaned == '': continue
#             industry_key = cleaned[:2] if len(cleaned) >= 2 else cleaned
#             group_key = f"{matched_group}_{industry_key}"
#             special_groups.setdefault(group_key, []).append({'code': code, 'money': money, 'group_type': matched_group})
#         except Exception:
#             continue

#     normal_groups = {}
#     for code in normal_qualified:
#         try:
#             original_name = get_stock_name(code)
#             money = normal_qualified[code]
#             cleaned = clean_name(original_name, is_special=False)
#             if cleaned == '': continue
#             industry_key = cleaned[:2] if len(cleaned) >= 2 else cleaned
#             normal_groups.setdefault(industry_key, []).append({'code': code, 'money': money, 'group_type': '普通'})
#         except Exception:
#             continue

#     final_pool_info = []
#     for items in special_groups.values():
#         final_pool_info.append(max(items, key=lambda x: x['money']))
...
#     top_100 = final_pool_info_sorted[:100]
#     g['dynamic_etf_pool'] = [item['code'] for item in top_100]
#     print(f" 动态池最终纳入: {len(g['dynamic_etf_pool'])}只")

# def filter_fixed_pool_by_volume(ContextInfo, cur_date):
#     if g.get('avg_etf_money_threshold') is None:
#         calculate_global_etf_threshold(ContextInfo, cur_date)
#     if not g['fixed_etf_pool']: return
#     dynamic_threshold = g['avg_etf_money_threshold']
#     try:
#         market_data = trader.get_daily_data_cached(['amount'], g['fixed_etf_pool'], count=3, end_time=cur_date)
#         avg_daily_money = {}
#         for code in g['fixed_etf_pool']:
#             if code in market_data and 'amount' in market_data[code]:
#                 avg_daily_money[code] = market_data[code]['amount'].mean()
#         qualified = {k: v for k, v in avg_daily_money.items() if v > dynamic_threshold}
#         g['filtered_fixed_pool'] = list(qualified.keys())
#         print(f"【固定池过滤】保留{len(g['filtered_fixed_pool'])}只")
#     except Exception as e:
#         print(f"【固定池过滤】异常: {e}")
#         g['filtered_fixed_pool'] = g['fixed_etf_pool'][:]

# def daily_merge_etf_pools(ContextInfo):
#     if not g.get('filtered_fixed_pool'):
#         g['filtered_fixed_pool'] = g['fixed_etf_pool'][:]
#     merged = list(set(g['filtered_fixed_pool'] + (g.get('dynamic_etf_pool', []))))
#     merged.sort()
#     g['merged_etf_pool'] = merged
...
# # 【动量计算】长期(25日) + 短期(21日)
# # ============================================================================
# def calculate_and_log_ranked_etfs(ContextInfo, cur_date: str, cur_time: str):
#     if not g.get('merged_etf_pool'):
#         g['ranked_etfs_result'] = []
#         return
#     final_list = get_final_ranked_etfs(ContextInfo, cur_date, cur_time)
#     g['ranked_etfs_result'] = final_list
...
# # ============================================================================
# def _whipsaw_global_period_ok():
#     if not g['whipsaw_options_only_in_range']:
#         return True
#     return g['market_regime'] == '震荡期'

# def _use_smoothed_momentum_prices():
#     if not g['enable_smoothed_momentum_input']:
#         return False
#     if not _whipsaw_global_period_ok():
#         return False
#     if g['smoothed_momentum_only_in_range']:
#         return g['market_regime'] == '震荡期'
#     return True

# def _use_range_long_momentum_limits():
#     if not g['enable_range_momentum_floor']:
#         return False
#     if not _whipsaw_global_period_ok():
#         return False
#     return g['market_regime'] == '震荡期'
...
# def _use_range_short_momentum_limits():
#     if not g['enable_range_short_momentum_limits']:
#         return False
#     if not _whipsaw_global_period_ok():
#         return False
#     return g['market_regime'] == '震荡期'
...
# def _dual_positive_filter_should_apply():
#     if not g['enable_dual_positive_momentum']:
#         return False
#     if not _whipsaw_global_period_ok():
#         return False
#     if g['dual_positive_only_in_range']:
#         return g['market_regime'] == '震荡期'
#     return True

# def _effective_r2_threshold():
#     base = g['r2_threshold']
#     if g['enable_p0_state_tuning_patch'] and g['market_regime'] == '正常期':
#         return g['normal_r2_threshold_override']
#     if not g['enable_range_r2_veto'] or not _whipsaw_global_period_ok():
#         return base
#     if g['market_regime'] != '震荡期':
#         return base
#     return g['r2_threshold_range_bound']

# def _effective_laplace_min_slope():
#     base = g['laplace_min_slope']
#     if g['enable_p0_state_tuning_patch'] and g['market_regime'] == '正常期':
#         return g['normal_laplace_min_slope_override']
#     return base

# def _effective_laplace_s():
#     if not g['enable_laplace_s_regime_differentiation']:
#         return g['laplace_s_param']
#     if g['market_regime'] == '正常期':
#         return g['laplace_s_param_normal']
#     if g['market_regime'] == '走弱期':
#         return g['laplace_s_param_weak']
#     return g['laplace_s_param']

# # ============================================================================
# # 【核心】get_final_ranked_etfs
# # ============================================================================
# def get_final_ranked_etfs(ContextInfo, cur_date: str, cur_time: str):
#     all_metrics = []
#     etf_set: List[str] = list(g['merged_etf_pool'])
#     end_date = trader.get_prev_trade_date(cur_date)
#     if not end_date:
#         return []
#     cur_dt = cur_date + cur_time
#     short_lb = g['short_momentum_lookback']
#     lookback = max(g['lookback_days'], short_lb, g['volume_lookback'], g['ma_lookback']) + 20
#     regime = g['market_regime']

#     print(f"【动量得分计算】池={len(etf_set)}只, 状态={regime}")

#     market_data = trader.get_daily_data_cached(['close', 'volume'], etf_set, count=lookback + 20, end_time=end_date)
#     if not market_data:
#         print("无法获取历史价格数据")
#         return []
...
#     try:
#         minute_data = ContextInfo.get_market_data_ex(['volume', 'close'], etf_set, period='1m',
#             start_time=cur_date + '093000', end_time=cur_dt, count=240, subscribe=True)
#         if minute_data:
...
#     for etf in etf_set:
#         if etf not in market_data: continue
#         data = market_data[etf]
#         if 'close' not in data or len(data['close']) < max(g['lookback_days'], short_lb): continue
...
#         if len(hist_closes) < max(g['lookback_days'], short_lb): continue

#         etf_name = get_stock_name(etf)
#         current_price = today_prices.get(etf)
#         if current_price is None or np.isnan(current_price):
#             current_price = trader.get_price(ContextInfo, etf)
#         today_vol = today_vols.get(etf, 0)

#         metrics = calculate_all_metrics_for_etf(etf, etf_name, hist_closes, hist_volumes, current_price, today_vol, ContextInfo)
#         if metrics:
#             if metrics['etf'] in {m['etf'] for m in all_metrics}: continue
#             all_metrics.append(metrics)
...
#     all_metrics.sort(key=lambda x: x.get('momentum_rank_score', float('-inf')), reverse=True)
#     g['full_ranked_etfs_result'] = all_metrics[:]

#     filtered_list = apply_filters(all_metrics)
#     filtered_list.sort(key=lambda x: x.get('momentum_score', float('-inf')), reverse=True)
#     top_10 = filtered_list[:10]
#     g['filtered_ranked_etfs_result'] = filtered_list[:10]
...
#     # 候选池
#     if len(top_10) >= g['holdings_num']:
#         reference_score = top_10[g['holdings_num'] - 1].get('momentum_rank_score', float('-inf'))
#         ratio = g['score_threshold_ratio'] if regime != '走弱期' else 1.0
#         score_threshold = reference_score * ratio
#         candidate_pool = [item for item in top_10 if item.get('momentum_rank_score', float('-inf')) >= score_threshold]
#     else:
#         candidate_pool = top_10[:]

#     current_holdings = list(holding_dict.keys()) if holding_dict else []
#     candidate_dict = {item['etf']: item for item in candidate_pool}

#     # 防频换逻辑
#     regime_allows_anti = (regime == '正常期' or (regime == '震荡期' and g['oscillation_anti_churn_enabled']) or (regime == '走弱期' and g['weak_anti_churn_enabled']))
#     if regime_allows_anti and g['holdings_num'] >= 1 and len(candidate_pool) > 0:
#         if g['holdings_num'] > 1:
#             # 多仓防频换
#             topk = max(1, g['holdings_num'])
#             max_days = g['oscillation_max_days_not_topk'] if regime == '震荡期' else g['normal_max_days_not_topk']
#             streaks = dict(g['normal_not_topk_streaks'] or {})
#             ranked_codes = [item['etf'] for item in filtered_list]
#             rank_map = {code: idx+1 for idx, code in enumerate(ranked_codes)}
#             pool_codes = set(candidate_dict.keys())
#             retained = []
#             for h in current_holdings:
#                 if h not in pool_codes or rank_map.get(h, 999) > topk:
#                     streaks[h] = streaks.get(h, 0) + 1
#                     if streaks[h] >= max_days:
#                         streaks.pop(h, None)
#                     else:
#                         m = candidate_dict.get(h) or next((x for x in all_metrics if x['etf'] == h), None)
#                         if m:
#                             retained.append(m)
#                 else:
#                     streaks[h] = 0
#                     retained.append(candidate_dict[h])
#             target_count = max(1, g['holdings_num'])
#             fill_pool = [item for item in candidate_pool if item['etf'] not in {r['etf'] for r in retained}]
#             final_result = retained + fill_pool[:max(0, target_count - len(retained))]
#             final_result = final_result[:target_count]
#             active_codes = {item['etf'] for item in final_result}
#             streaks = {k: v for k, v in streaks.items() if k in active_codes}
...
#         else:
#             # 单仓防频换
#             max_days = g['normal_max_days_not_rank1']
#             new_first = candidate_pool[0]
#             pool_codes = set(candidate_dict.keys())
#             H = current_holdings[0] if len(current_holdings) == 1 else (current_holdings[0] if current_holdings else None)
#             rH = None
...
#                         rH = i + 1
#                         break
#             if not H or H not in pool_codes or rH is None or rH > 1:
#                 final_result = [new_first]
#                 g['normal_not_rank1_streak'] = 0
#                 g['normal_streak_hold_code'] = new_first['etf']
#             else:
...
#                     g['normal_not_rank1_streak'] = 0
#                 g['normal_streak_hold_code'] = H
#                 g['normal_not_rank1_streak'] += 1
#                 if g['normal_not_rank1_streak'] >= max_days:
#                     final_result = [new_first]
#                     g['normal_not_rank1_streak'] = 0
#                     g['normal_streak_hold_code'] = new_first['etf']
#                 else:
...
#         retained = [candidate_dict[etf] for etf in current_holdings if etf in candidate_dict]
#         if len(retained) >= g['holdings_num']:
#             retained_sorted = sorted(retained, key=lambda x: x.get('momentum_rank_score', float('-inf')), reverse=True)
#             final_result = retained_sorted[:g['holdings_num']]
#         else:
#             need = g['holdings_num'] - len(retained)
#             remaining_pool = [item for item in candidate_pool if item['etf'] not in {r['etf'] for r in retained}]
...
#     # 滞回
#     if g['enable_switch_hysteresis'] and g['holdings_num'] == 1 and final_result:
#         hs_current = current_holdings[0] if current_holdings else None
#         hs_target = final_result[0]['etf']
#         if hs_current and hs_current != hs_target:
#             in_range = regime == '震荡期'
#             hbuf = g['switch_buffer_range'] if in_range else g['switch_buffer_normal']
#             hs_cur_metric = next((m for m in filtered_list if m['etf'] == hs_current), None)
#             if hs_cur_metric:
#                 t_sc = final_result[0].get('momentum_rank_score', float('-inf'))
#                 c_sc = hs_cur_metric.get('momentum_rank_score', float('-inf'))
#                 if t_sc <= c_sc * (1.0 + hbuf):
#                     final_result = [hs_cur_metric]
...
# # 指标计算 & 过滤
# # ============================================================================
# def calculate_all_metrics_for_etf(etf, etf_name, hist_closes, hist_volumes, current_price, today_vol, ContextInfo):
#     try:
...
#         effective_r2 = _effective_r2_threshold()
#         passed_r2 = r_squared > effective_r2

#         # 动量得分区间
#         eff_min = g['min_score_threshold']
#         eff_max = g['max_score_threshold']
#         if _use_range_long_momentum_limits():
#             eff_min = max(eff_min, g['range_momentum_min'])
#             eff_max = min(eff_max, g['range_momentum_max'])

#         momentum_rank_score = momentum_score
#         momentum_soft_capped = False
#         apply_soft_cap = g['enable_momentum_soft_cap'] and ((not g['momentum_soft_cap_normal_only']) or g['market_regime'] == '正常期')
#         if apply_soft_cap and momentum_score > eff_max:
#             penalty = max(0.0, min(1.0, g['momentum_soft_cap_penalty']))
#             momentum_rank_score = eff_max + (momentum_score - eff_max) * penalty
#             momentum_soft_capped = True
#             passed_momentum = momentum_score >= eff_min
#         else:
#             passed_momentum = (eff_min <= momentum_score <= eff_max)

#         eff_short_min = 0
#         eff_short_max = 6
#         if _use_range_short_momentum_limits():
#             eff_short_min = max(eff_short_min, g['range_short_momentum_min'])
#             eff_short_max = min(eff_short_max, g['range_short_momentum_max'])
#         passed_short_momentum = (eff_short_min <= short_momentum_score <= eff_short_max) if short_momentum_score is not None else False
#         passed_whipsaw_short_band = passed_short_momentum if _use_range_short_momentum_limits() else True
...
#         volume_ratio = None
#         try:
#             if hist_volumes is not None and len(hist_volumes) >= g['volume_lookback']:
#                 past_vol = hist_volumes[-g['volume_lookback']:]
#                 if not (np.any(np.isnan(past_vol)) or np.any(past_vol == 0)):
#                     avg_vol = np.mean(past_vol)
#                     if avg_vol > 0:
#                         curdatetime = timetag_to_datetime(ContextInfo.get_bar_timetag(ContextInfo.barpos), '%Y%m%d%H%M%S')
#                         hour = int(curdatetime[8:10])
#                         minute = int(curdatetime[10:12])
#                         elapsed = (hour - 9) * 60 + minute - 30
#                         if hour >= 13:
#                             elapsed -= 90
#                         elapsed = max(1, min(elapsed, 240))
#                         projected = today_vol * (240.0 / elapsed)
#                         volume_ratio = projected / avg_vol
#         except Exception:
#             pass
...
#         day_ratios = []
#         if len(price_series) >= 4:
#             day_ratios = [price_series[-1]/price_series[-2], price_series[-2]/price_series[-3], price_series[-3]/price_series[-4]]
#             if min(day_ratios) < g['loss']:
#                 passed_loss_filter = False

#         # 均线过滤
#         passed_ma = True
#         if g['enable_ma_filter'] and g['market_regime'] == '走弱期' and len(price_series) >= g['ma_lookback']:
#             ma_value = np.mean(price_series[-g['ma_lookback']:])
#             passed_ma = current_price > ma_value * g['ma_threshold']
#         elif g['market_regime'] != '走弱期':
#             passed_ma = True

#         # 拉普拉斯滤波
#         laplace_slope = 0
#         passed_laplace = False
#         eff_laplace_s = _effective_laplace_s()
#         if g['enable_laplace_filter'] and len(price_series) >= 10:
#             alpha = 1 - np.exp(-eff_laplace_s)
#             L = np.zeros(len(price_series))
#             L[0] = price_series[0]
#             for t in range(1, len(price_series)):
#                 L[t] = alpha * price_series[t] + (1 - alpha) * L[t-1]
#             laplace_slope = L[-1] - L[-2] if len(L) >= 2 else 0
#             laplace_min = _effective_laplace_min_slope()
#             passed_laplace = (current_price > L[-1] and laplace_slope > laplace_min)

#         # 高斯滤波（震荡期专用）
#         gaussian_slope = 0
#         passed_gaussian = False
#         if g['market_regime'] == '震荡期' and len(price_series) >= 10:
#             try:
#                 n = len(price_series)
#                 sig = g['gaussian_sigma']
#                 idx1 = np.arange(n)
#                 w1 = np.exp(-((idx1 + 1) ** 2) / (2 * sig ** 2))[::-1]
#                 w1 /= np.sum(w1)
#                 g1 = np.sum(price_series * w1)
#                 price2 = price_series[:-1]
#                 idx2 = np.arange(n-1)
#                 w2 = np.exp(-((idx2 + 1) ** 2) / (2 * sig ** 2))[::-1]
#                 w2 /= np.sum(w2)
#                 g2 = np.sum(price2 * w2)
#                 if g['gaussian_use_relative_slope']:
#                     gaussian_slope = ((g1 - g2) / g2) if abs(g2) > 1e-12 else 0.0
#                     g_min = g['gaussian_min_slope_relative']
#                 else:
#                     gaussian_slope = g1 - g2
...
#             try:
#                 iopv = get_etf_iopv(etf)
#                 iopv = float(iopv)
#                 if iopv > 0:
#                     premium_rate = (current_price - iopv) / iopv * 100
#                     passed_premium = premium_rate <= g['max_premium_rate']
#             except Exception:
#                 pass
...
#             'momentum_rank_score': momentum_rank_score, 'momentum_soft_capped': momentum_soft_capped,
#             'short_momentum_score': short_momentum_score, 'annualized_returns': annualized_returns,
#             'r_squared': r_squared, 'current_price': current_price,
#             'volume_ratio': volume_ratio, 'day_ratios': day_ratios, 'premium_rate': premium_rate,
#             'passed_momentum': passed_momentum, 'passed_r2': passed_r2, 'passed_ma': passed_ma,
#             'passed_volume': passed_volume, 'passed_loss': passed_loss_filter,
#             'passed_premium': passed_premium, 'passed_laplace': passed_laplace,
#             'passed_gaussian': passed_gaussian, 'laplace_slope': laplace_slope,
#             'gaussian_slope': gaussian_slope, 'effective_r2_threshold': effective_r2,
#             'smoothed_momentum_used': smoothed_used,
#             'passed_whipsaw_short_band': passed_whipsaw_short_band,
#             'passed_dual_positive': passed_dual_positive, 'dual_positive_filter_active': dual_pos_active,
#         }
#     except Exception as e:
#         if not ContextInfo.do_back_test:          # ← 新增条件判断
#             print(f"【指标计算】{etf} {etf_name} 计算失败: {e}")
#         return None

# def apply_filters(metrics_list):
#     regime = g['market_regime']
#     is_weak = regime == '走弱期'
#     is_oscillation = regime == '震荡期'
#     steps = [
#         ('动量得分', lambda m: m['passed_momentum'], True),
#         ('R2', lambda m: m['passed_r2'], g['enable_r2_filter'] and (not is_weak or g['weak_apply_r2_filter'])),
#         ('均线', lambda m: m['passed_ma'], g['enable_ma_filter'] and is_weak and g['weak_apply_ma_filter']),
#         ('成交量', lambda m: m['passed_volume'], g['enable_volume_check'] and (not is_weak or g['weak_apply_volume_filter'])),
#         ('短期风控', lambda m: m['passed_loss'], g['enable_loss_filter'] and (not is_weak or g['weak_apply_loss_filter'])),
#         ('溢价率', lambda m: m['passed_premium'], g['enable_premium_filter'] and (not is_weak or g['weak_apply_premium_filter'])),
#         ('拉普拉斯', lambda m: m['passed_laplace'], g['enable_laplace_filter'] and (regime == '正常期' or (is_weak and g['weak_apply_laplace_filter']))),
#         ('高斯滤波', lambda m: m['passed_gaussian'], is_oscillation),
#         ('震荡期短期动量区间', lambda m: m.get('passed_whipsaw_short_band', True), True),
#         ('长短动量双正', lambda m: (not m.get('dual_positive_filter_active', False)) or m.get('passed_dual_positive', False), True),
#     ]
#     filtered = metrics_list[:]
#     for name, condition, is_enabled in steps:
#         if is_enabled:
...
#                 print(f"【过滤条件】{name}: 通过 {after_count}/{before_count}")
#     return filtered

# def get_volume_ratio(hist_volumes, today_vol, ContextInfo, lookback_days=None):
#     if lookback_days is None:
#         lookback_days = g['volume_lookback']
#     try:
#         if hist_volumes is None or len(hist_volumes) < lookback_days:
...
# # 交易执行
# # ============================================================================
# def execute_sell_trades(ContextInfo):
#     ranked_etfs = g.get('ranked_etfs_result', [])
#     target_etfs = []
#     if ranked_etfs:
#         for metrics in ranked_etfs[:g['holdings_num']]:
#             target_etfs.append(metrics['etf'])
...
#         for security, pos in holdings.items():
#             if pos.m_nVolume > 0:
#                 current_positions.append(security)
#     except:
#         pass
...
#             stock_name = get_stock_name(security)
#             success = smart_order_target_value(security, 0, ContextInfo)
#             if success:
#                 print(f"已成功卖出: {security} {stock_name}")
#             else:
#                 print(f"卖出失败: {security} {stock_name}")
...
#     num_etfs_to_buy = min(len(etfs_to_buy), max_buy_count)
#     if num_etfs_to_buy <= 0:
#         print(f"当前持仓({actual_holding_count})已达到或超过目标({g['holdings_num']})，无需买入")
#         return
#     etfs_to_buy = etfs_to_buy[:num_etfs_to_buy]
#     print(f"当前持仓: {actual_holding_count}只, 目标: {g['holdings_num']}只, 本次买入: {num_etfs_to_buy}只")
#     available_cash = trader.get_strategy_available_cash()
#     allocated_value_per_etf = available_cash // num_etfs_to_buy
#     if allocated_value_per_etf < g['min_money']:
#         print(f"单只分配金额{allocated_value_per_etf:.2f}不足最小交易额，无法买入")
#         return
#     for i, etf in enumerate(etfs_to_buy):
#         target_value = allocated_value_per_etf
#         if i == len(etfs_to_buy) - 1:
#             target_value = available_cash
#         stock_name = get_stock_name(etf)
#         success = smart_order_target_value(etf, target_value, ContextInfo)
#         if success:
#             print(f"买入成功: {etf} {stock_name}")

# def smart_order_target_value(security, target_value, ContextInfo):
#     try:
#         account_id = trader.acct_id
#         stock_name = get_stock_name(security)

#         if not ContextInfo.do_back_test:
#             tick_data = ContextInfo.get_full_tick([security])
#             if not tick_data or security not in tick_data:
#                 print(f"{security} {stock_name}: 获取行情失败")
#                 return False
#             tick = tick_data[security]
#             if not isinstance(tick, dict):
#                 return False
#             if tick.get('status', '') in ['停牌', 'ST停牌']:
#                 print(f"{security} {stock_name}: 停牌")
#                 return False
...
#             if last_close > 0:
#                 high_limit = last_close * 1.1
#                 low_limit = last_close * 0.9
#             else:
#                 high_limit = current_price * 1.1
#                 low_limit = current_price * 0.9
#             if current_price >= high_limit:
#                 print(f"{security} {stock_name}: 涨停")
#                 return False
#             if current_price <= low_limit and target_value > 0:
#                 print(f"{security} {stock_name}: 跌停")
#                 return False
#         else:
#             tick = {}
...
#         if amount_diff < 0:
#             if closeable_amount == 0:
#                 print(f"{security} {stock_name}: T+1不可卖")
#                 return False
#             amount_diff = -min(abs(amount_diff), closeable_amount)
...
#                     passorder(23, 1101, account_id, security, 11, current_price, amount_diff, trader.strategy_name, 1, '', ContextInfo)
#                 else:
#                     smart_passorder_market(23, account_id, security, amount_diff, current_price, tick, ContextInfo)
#                 print(f"买入{security} {stock_name}，数量: {amount_diff}，价格: {current_price:.3f}")
#             else:
#                 if ContextInfo.do_back_test:
#                     passorder(24, 1101, account_id, security, 11, current_price, abs(amount_diff), trader.strategy_name, 1, '', ContextInfo)
#                 else:
#                     smart_passorder_market(24, account_id, security, abs(amount_diff), current_price, tick, ContextInfo)
...
# def smart_passorder_market(opType: int, account_id: str, code: str, amount: int, current_price: float, tick: dict, ContextInfo):
#     market = _get_market_type(code)
...
#         high_limit = current_price * 1.1
#         low_limit = current_price * 0.9
#     protect_price = high_limit if opType == 23 else low_limit
#     if protect_price <= 0:
#         protect_price = current_price * 1.1 if opType == 23 else current_price * 0.9
#     now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#     user_order_id = f'[{now_str}][五福v5.1][{code}][{"buy" if opType == 23 else "sell"}] {amount}'
#     passorder(opType, 1101, account_id, code, prType, protect_price, amount, trader.strategy_name, 1, user_order_id, ContextInfo)
#     return True

# # ============================================================================
# # 止损
# # ============================================================================
# def minute_level_stop_loss(ContextInfo, cur_time: str):
...
#                 continue
#             current_price = trader.get_price(ContextInfo, security) or 0
...
#                 stock_name = get_stock_name(security)
#                 loss_percent = (current_price / cost_price - 1) * 100
#                 print(f"[ALERT] 【止损】{security} {stock_name} 亏损: {loss_percent:.2f}%")
#                 smart_order_target_value(security, 0, ContextInfo)
#     except Exception as e:
#         print(f"【止损】异常: {e}")

# def minute_level_pct_stop_loss(ContextInfo, cur_date: str, cur_time: str):
#     if not g['use_pct_stop_loss'] or not trader.is_trading_time(ContextInfo, cur_time):
#         return
#     try:
#         holdings = trader.get_holdings()
#         for security, pos in holdings.items():
...
#             if yesterday_close is None:
#                 end_date = trader.get_prev_trade_date(cur_date)
#                 market_data = trader.get_daily_data_cached(['close'], [security], count=1, end_time=end_date)
#                 if market_data and security in market_data and 'close' in market_data[security]:
#                     yesterday_close = market_data[security]['close'].iloc[-1]
...
#             if current_price <= stop_price:
#                 stock_name = get_stock_name(security)
#                 print(f"[ALERT] 【跌幅止损】{security} {stock_name}")
#                 smart_order_target_value(security, 0, ContextInfo)
#     except Exception as e:
#         print(f"【跌幅止损】异常: {e}")
...
# # 辅助函数
# # ============================================================================
# def download_etf_his_data(cur_date: str, ContextInfo):
#     if g.get('history_last_dl_date') == cur_date: return
#     if ContextInfo.do_back_test and g.get('history_last_dl_date'): return
#     etf_list = trader.get_valid_stock_list(['沪深ETF'])
#     if ContextInfo.do_back_test: etf_list = g['fixed_etf_pool']
#     if g['dl_history_data']:
#         # 1. 下载指数日线（数量少，直接逐条打印）
#         idx_codes = list(g['regime_indexes'].values())
#         for code in idx_codes:
#             download_history_data(code, '1d', '', '')
#             print(f" 已下载指数 {code} 日线")

#         # 2. 下载 ETF 数据（1分钟 + 日线），每 10 个输出一次进度
#         total = len(etf_list)
#         for idx, etf in enumerate(etf_list, 1):
#             download_history_data(etf, '1m', '', '')
#             download_history_data(etf, '1d', '', '')
#             if idx % 10 == 0 or idx == total:
#                 print(f" 历史数据下载进度: {idx}/{total} ({100*idx//total}%)")
#     # 预加载缓存
#     _ = trader.get_daily_data_cached([], etf_list, count=500)
#     g['history_last_dl_date'] = cur_date

# def get_stock_name(symbol) -> str:
#     return trader.get_stock_info(symbol, 'InstrumentName')

# def check_defensive_etf_available(ContextInfo):
#     defensive_etf = g['defensive_etf']
#     try:
#         if ContextInfo.do_back_test:
#             return trader.get_price(ContextInfo, defensive_etf) is not None
#         tick_data = ContextInfo.get_full_tick([defensive_etf])
#         if defensive_etf not in tick_data: return False
#         tick = tick_data[defensive_etf]
#         if tick.get('status') in ['停牌', 'ST停牌']: return False
...
#             low_limit = last_close * 0.9
#         else:
#             high_limit = last_price * 1.1
#             low_limit = last_price * 0.9
#         if last_price >= high_limit: return False
#         if last_price <= low_limit: return False
#         return True
#     except:
#         return False
