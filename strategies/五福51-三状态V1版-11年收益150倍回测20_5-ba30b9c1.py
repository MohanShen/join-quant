...
# postId: ba30b9c1a934b17058ba0065e6408507
# backtestId: 64483aecb130687e4aff8355abf9fd8e
# title: 五福51-三状态V1版-11年收益150倍回测20.5
...
#行业ETF：
    ...
    # 正常期：未重返动量第1名_MAX 个交易日则强制换股（可配置）；仅在 holdings_num==1 时生效
...
def record_etf_roundtrip_on_sell(context, security, sold_amount, avg_cost_before, sell_price, exit_reason):
    ...
    log.info(
        f"\n{'=' * 72}\n"
        f"📋 【卖出复盘】{name}({security})  |  原因: {exit_reason}\n"
...
def get_final_ranked_etfs(context):
    ...
    regime_allows_anti_churn = regime == '正常期' or (
        regime == '震荡期' and getattr(g, 'oscillation_anti_churn_enabled', False)
    )
    ...
    if use_regime_anti_churn:
        max_days = int(getattr(g, 'normal_max_days_not_rank1', 5))
        AC = '【震荡期防频换】' if regime == '震荡期' else '【正常期防频换】'
        ...
        if len(current_holdings) > 1:
            w = f"{AC}持仓{len(current_holdings)}只，仅对首只{H}应用防频换"
            log_buffer.append(w)
            log.info(w)
...
def minute_level_stop_loss(context):
    ...
    for security in list(context.portfolio.positions.keys()):
        ...
        if current_price <= cost_price * g.fixedStopLossThreshold:
            ...
            smart_order_target_value(security, 0, context, exit_reason='分钟固定止损')
...
def minute_level_pct_stop_loss(context):
    ...
    for security in list(context.portfolio.positions.keys()):
        ...
        if current_price <= stop_price:
            ...
            smart_order_target_value(security, 0, context, exit_reason='分钟跌幅止损')
