...
# postId: 872daacb217dae83c158e0056a30e628
# backtestId: ade11dee68c2afa230db35622fedb4e6
# title: 五福V5.0 调优记录思考-趋势狙击手，死于装备过载
...
#行业ETF：
    ...
    # 防未来函数/防拟合口径：交易信号只使用上一交易日已确认数据。
    # 实盘仍按13:10实时价成交，但排序、过滤和弱势判断不混入当日未收盘数据。
    # g.use_previous_close_for_signal = True
    # g.use_intraday_volume_projection = False
    g.use_previous_close_for_signal = False
    g.use_intraday_volume_projection = True
...
=== 防未来函数口径 ===
- 信号价格: {'上一交易日收盘价' if g.use_previous_close_for_signal else '交易时点实时价'}
- 成交量过滤: {'上一交易日成交量/此前均量' if not g.use_intraday_volume_projection else '日内成交量外推'}
=== 其他配置 ===
...
def get_signal_date(context):
    """返回信号计算可见的最新完整交易日。"""
    return context.previous_date if context.previous_date is not None else context.current_dt.date()


def get_visible_etf_pool(etf_codes, context, pool_name="ETF池"):
    """按信号日过滤ETF池，避免历史回测使用未来才上市的ETF代码。"""
    if not etf_codes:
        return []
    signal_date = get_signal_date(context)
    try:
        df_etf = get_all_securities(['etf'], date=signal_date)
        visible_codes = set(df_etf.index.tolist())
        visible_pool = [code for code in etf_codes if code in visible_codes]
        if hasattr(g, 'etf_names_dict'):
            g.etf_names_dict.update(df_etf['display_name'].to_dict())
        removed_count = len(etf_codes) - len(visible_pool)
        if removed_count > 0:
            log.info(f"【{pool_name}】按{signal_date}可见ETF过滤: {len(etf_codes)}→{len(visible_pool)}只，剔除未来/不可见标的{removed_count}只")
        return visible_pool
    except Exception as e:
        log.warning(f"【{pool_name}】按历史日期过滤ETF池失败: {e}，使用原始池")
        return etf_codes[:]
...
def afternoon_routine(context):
    ...
    if g.is_a_share_weak:
        if hasattr(g, 'filtered_global_pool') and g.filtered_global_pool:
            ...
            g.merged_etf_pool = list(set(get_visible_etf_pool(g.global_etf_pool, context, "全球/海外ETF池")))
...
def calculate_global_etf_threshold(context):
    ...
    try:
        ...
        if not etf_list:
            ...
            return
        log.info(f"{signal_date}可见全市场ETF总数: {len(etf_list)}只")
        trade_days = get_trade_days(end_date=signal_date, count=3)
        start_day = trade_days[0]
        df = get_price(security=etf_list, start_date=start_day, end_date=signal_date, frequency='daily', fields=['money'], panel=False, skip_paused=True)
...
def filter_global_pool_by_volume(context):
    ...
    end_date = get_signal_date(context)
    TRADE_DAYS_COUNT = 3
    try:
        visible_global_pool = get_visible_etf_pool(g.global_etf_pool, context, "全球/海外ETF池")
        if not visible_global_pool:
            log.warning("【全球池过滤】信号日无可见全球/海外ETF，过滤结果为空")
            g.filtered_global_pool = []
            return
        price_data = get_price(visible_global_pool, end_date=end_date, count=TRADE_DAYS_COUNT, frequency='daily', fields=['money'], panel=False)
        if price_data is None or price_data.empty:
            log.warning("【全球池过滤】无法获取成交额数据，使用原始全球池")
            g.filtered_global_pool = visible_global_pool[:]
            return
        ...
        removed = set(visible_global_pool) - set(new_global_pool)
        if removed:
            removed_info = []
            for code in removed:
                try:
        ...
        g.filtered_global_pool = get_visible_etf_pool(g.global_etf_pool, context, "全球/海外ETF池")
...
def update_sector_pool(context):
    ...
    exclude_keywords = sorted(list(set([
        ...
        '短融', '可转债', '转债', '双债', '利率债', '国债', '地债', '政���债', '国开债', '基准国债', '新综债',
    ...
    log.info(f"【动态池更新】{get_signal_date(context)}可见全市场ETF总数: {len(etf_list)}只")
    normal_etfs = []
    special_etfs = []
    special_group_map = {}
    excluded_count = 0
...
def filter_fixed_pool_by_volume(context):
    ...
    TRADE_DAYS_COUNT = 3
    try:
        visible_fixed_pool = get_visible_etf_pool(g.fixed_etf_pool, context, "固定ETF池")
        if not visible_fixed_pool:
            log.warning("【固定池过滤】信号日无可见固定ETF，过滤结果为空")
            g.filtered_fixed_pool = []
            return
        price_data = get_price(visible_fixed_pool, end_date=end_date, count=TRADE_DAYS_COUNT, frequency='daily', fields=['money'], panel=False)
        if price_data is None or price_data.empty:
            log.warning("【固定池过滤】无法获取成交额数据，跳过过滤")
            g.filtered_fixed_pool = visible_fixed_pool[:]
            return
        total_money = price_data.groupby('code')['money'].sum()
        avg_daily_money = total_money / TRADE_DAYS_COUNT
        qualified = avg_daily_money[avg_daily_money > dynamic_threshold]
        new_fixed_pool = qualified.index.tolist()
        removed = set(visible_fixed_pool) - set(new_fixed_pool)
        if removed:
            removed_info = []
            for code in removed:
                try:
        ...
        g.filtered_fixed_pool = get_visible_etf_pool(g.fixed_etf_pool, context, "固定ETF池")


def daily_merge_etf_pools(context):
    if not hasattr(g, 'filtered_fixed_pool'):
        g.filtered_fixed_pool = get_visible_etf_pool(g.fixed_etf_pool, context, "固定ETF池")
    merged = list(set(get_visible_etf_pool(g.filtered_fixed_pool, context, "固定ETF池") + get_visible_etf_pool(g.dynamic_etf_pool, context, "动态ETF池")))
    merged.sort()
...
def get_volume_ratio(hist_volumes, today_vol, context, lookback_days=None):
    ...
    try:
        if not getattr(g, 'use_intraday_volume_projection', True):
            if hist_volumes is None or len(hist_volumes) < lookback_days + 1:
                return None
            signal_day_vol = hist_volumes[-1]
            past_n_days_vol = hist_volumes[-(lookback_days + 1):-1]
            if signal_day_vol <= 0 or np.any(np.isnan(past_n_days_vol)) or np.any(past_n_days_vol == 0):
                return None
            avg_volume = np.mean(past_n_days_vol)
            return signal_day_vol / avg_volume if avg_volume > 0 else None
...
def calculate_premium_rate(etf, context):
    try:
        ...
        if etf_price is None or pd.isna(etf_price):
            etf_price_df = get_price(etf, start_date=signal_date, end_date=signal_date, fields=['close'])
        ...
        if nav is None or pd.isna(nav):
            nav_df = get_extras('unit_net_value', etf, start_date=signal_date, end_date=signal_date)
...
def check_a_share_weak_period(context):
    ...
    for name, code in indexes.items():
        df = get_price(code, end_date=signal_date, count=g.weak_period_ma_lookback + 1,
                       frequency='daily', fields=['close'], panel=False, skip_paused=False)
        ...
        log.info(f"📊 【走弱期判断】{name}({code}) {signal_date}收盘{current_price:.2f} / MA{g.weak_period_ma_lookback} {ma_val:.2f} → {status_emoji}")
...
def get_final_ranked_etfs(context):
    all_metrics = []
    etf_set = get_visible_etf_pool(list(g.merged_etf_pool), context, "动量计算池")
    end_date = get_signal_date(context)
    log.info(f"【动量得分计算】使用合并池，合计{len(etf_set)}只ETF")
    log.info(f"【当前状态】{'🔴 大A走弱期' if g.is_a_share_weak else '🟢 大A正常期'}")
    log.info(f"【信号口径】使用{end_date}已收盘数据计算排序与过滤")
    if not etf_set:
        log.warning("【动量计算】信号日无可见ETF，无法计算")
        return []
    ...
    if getattr(g, 'use_intraday_volume_projection', True):
    ...
    for etf in etf_set:
        ...
        min_history = g.lookback_days + 1 if getattr(g, 'use_previous_close_for_signal', True) else g.lookback_days
        if len(hist_closes) < min_history:
            continue
        etf_name = get_security_name(etf)
        if getattr(g, 'use_previous_close_for_signal', True):
            current_price = hist_closes[-1]
            metric_hist_closes = hist_closes[:-1]
        else:
            current_price = current_data[etf].last_price
            metric_hist_closes = hist_closes
        today_vol = today_vols.get(etf, 0)
        metrics = calculate_all_metrics_for_etf(etf, etf_name, metric_hist_closes, hist_volumes, current_price, today_vol, context)
        if metrics:
    ...
    if len(retained) >= g.holdings_num:
        ...
        if additional:
            log_buffer.append("补充��ETF（按动量得分排序）：")
