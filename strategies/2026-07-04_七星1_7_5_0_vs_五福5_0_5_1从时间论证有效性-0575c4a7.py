...
# backtestId: 0b78cd3c49e0de041d57ec5194a34e7e
# title: 七星1.7、5.0 vs 五福5.0、5.1从时间论证有效性

# 克隆自聚宽文章：https://www.joinquant.com/post/72171
# 标题：7年44倍!七星V1.7.3 回撤22.7%→20.7%
# 作者：nick hu
...
# 作者：king088
#
# DELTA: H6 — 盈利保护卖出后增加买入冷却期(2 交易日空仓)
#   1) initialize 新增 g.protection_cooldown_days / g.protection_cooldown_remaining
#   2) profit_protection_check 卖出时置冷却计数
#   3) etf_buy_trade 冷却期内跳过买入(保持空仓, 避免回补进下跌市场)
...
def initialize(context):
    ...
    # H6: 盈利保护卖出后的买入冷却
    g.protection_cooldown_days = 2                         # 冷却交易日数
    g.protection_cooldown_remaining = 0                    # 冷却剩余计数（运行时）
        ...
        if pos.total_amount > 0:
            if check_profit_protection(sec, context):
                if smart_order_target_value(sec, 0, context):
                    ...
                    # H6: 触发买入冷却期
                    g.protection_cooldown_remaining = g.protection_cooldown_days
                    log.info(f"🧊 盈利保护触发买入冷却：{g.protection_cooldown_days} 个交易日")
                    # 触发止损信号，用于震荡期进入判断
...
def etf_buy_trade(context):
    ...
    # H6: 盈利保护冷却期内跳过买入，保持空仓
    if getattr(g, 'protection_cooldown_remaining', 0) > 0:
        g.protection_cooldown_remaining -= 1
        log.info(f"🧊 盈利保护冷却期，跳过买入（剩余 {g.protection_cooldown_remaining} 个交易日）")
        log.info("========== 买入操作完成 ==========")
        return
