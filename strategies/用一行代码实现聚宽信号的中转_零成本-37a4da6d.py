...
# postId: 37a4da6daee27396669b2bbb779246bc
# backtestId: 0d2bba53d5f214b810a73947b293e1e7
# title: 用一行代码实现聚宽信号的中转(零成本)

# 策略名称：三马105+五福35
# 克隆自聚宽文章：https://www.joinquant.com/post/70738
# 标题：三马105-五福35-修复版-11年收益659倍回撤14%
# 作者：rbq2025
...
def setup_wufu35_globals():
    ...
    g.fixed_etf_pool = [
        '588790.XSHG', '588220.XSHG', '588200.XSHG', '588170.XSHG', '588080.XSHG', '563300.XSHG', '562800.XSHG',
        '562590.XSHG', '562500.XSHG', '561980.XSHG', '561360.XSHG', '561330.XSHG', '560860.XSHG', '520830.XSHG',
        '518880.XSHG', '517520.XSHG', '516520.XSHG', '516510.XSHG', '516190.XSHG', '516160.XSHG', '516150.XSHG',
        '515980.XSHG', '515880.XSHG', '515790.XSHG', '515400.XSHG', '515220.XSHG', '515210.XSHG', '515170.XSHG',
        '515120.XSHG', '515050.XSHG', '515030.XSHG', '513970.XSHG', '513920.XSHG', '513750.XSHG', '513630.XSHG',
        '513520.XSHG', '513500.XSHG', '513400.XSHG', '513360.XSHG', '513350.XSHG', '513330.XSHG', '513310.XSHG',
        '513290.XSHG', '513190.XSHG', '513180.XSHG', '513120.XSHG', '513100.XSHG', '513090.XSHG', '513030.XSHG',
        '512980.XSHG', '512890.XSHG', '512880.XSHG', '512800.XSHG', '512710.XSHG', '512690.XSHG', '512670.XSHG',
        '512660.XSHG', '512480.XSHG', '512400.XSHG', '512200.XSHG', '512170.XSHG', '512100.XSHG', '512070.XSHG',
        '512050.XSHG', '512010.XSHG', '511380.XSHG', '510900.XSHG', '510880.XSHG', '510760.XSHG', '510500.XSHG',
        '510300.XSHG', '501018.XSHG', '161226.XSHE', '159998.XSHE', '159995.XSHE', '159992.XSHE', '159985.XSHE',
        '159981.XSHE', '159980.XSHE', '159967.XSHE', '159949.XSHE', '159928.XSHE', '159915.XSHE', '159892.XSHE',
        '159883.XSHE', '159870.XSHE', '159869.XSHE', '159865.XSHE', '159852.XSHE', '159851.XSHE', '159840.XSHE',
        '159825.XSHE', '159819.XSHE', '159766.XSHE', '159755.XSHE', '159732.XSHE', '159667.XSHE', '159638.XSHE',
        '159611.XSHE', '159605.XSHE', '159583.XSHE', '159566.XSHE', '159529.XSHE', '159518.XSHE', '159516.XSHE',
        '159509.XSHE', '159502.XSHE', '159363.XSHE', '159326.XSHE', '159323.XSHE', '159256.XSHE', '159227.XSHE',
        '159218.XSHE', '159206.XSHE',
    ]
...
def update_sector_pool(context):
    ...
    FUND_COMPANIES = sorted(list(set([
        ...
        '银华', '天弘', '建信', '工银', '华泰柏瑞', '博��', '景顺长城', '景顺', '华宝', '申万菱信', '万家', '中欧',
    ...
    SPECIAL_GROUPS = sorted([
        {'name': '香港组',
         'keywords': sorted(['恒生', '恒指', '港股', '港股通', 'H股', '香港', '港', 'HKC', 'HK', 'HGS', 'H', '中概', 'HS科技'], key=len,
                            reverse=True),
         'remove_words': sorted(['恒生', '恒指', '港股', '港股通', 'H股', '香港', '港', 'HKC', 'HK', 'HGS', 'H', '中概', 'HS'],
                                key=len, reverse=True)},
        {'name': '科创组', 'keywords': sorted(['科创', '科创板', '科综', 'KC', 'K C', '双创', '科创创业', '创创'], key=len, reverse=True),
         'remove_words': sorted(
             ['科创', '科创板', '科综', 'KC', 'K C', '双创', '科创创业', '创创', '债券', '债汇', '债指', '债沪', '债易', '债基', '债兴', '债摩', '债',
              'AAA'], key=len, reverse=True)},
...
def get_small_cap_stocks_v3(context):
    ...
    q = (
        query(
            ...
            .filter(
            valuation.code.in_(initial_list),
            valuation.market_cap.between(20, 200),  # 这里修改小市值的盘子范围
            income.operating_revenue > 1e8,
            indicator.roe > 0,
            indicator.roa > 0,
...
# v4 活跃小盘+量能放大 (国九+roa+roe)基础上微调
def get_small_cap_stocks_v4(context):
    # 1. 初始池
    initial_list = filter_stocks(context, get_index_stocks("399101.XSHE"))
    log.info(f"[V4] 初始池股票数量: {len(initial_list)}")
    if not initial_list:
        return [g.xsz_buy_etf]

    # 2. 计算一年涨停次数（历史数据，避免未来信息）
    limit_up_count = get_limit_up_count_batch(context, initial_list, days=250)
    log.info(f"[V4] 涨停次数统计完成，覆盖 {len(limit_up_count)} 只")
    ...
    q = (
        query(
            ...
            .filter(
            valuation.code.in_(initial_list),
            valuation.market_cap.between(10, 300),
            income.net_profit > 0,
            income.operating_revenue > 1e8,
            valuation.pe_ratio > 0,
            valuation.pe_ratio < 100,
            valuation.pb_ratio < 15,
            balance.total_liability / balance.total_assets < 0.8,
        )
    )
    df = get_fundamentals(q)
    log.info(f"[V4] 基本面筛选后: {len(df)} 只")
    if df.empty:
        return [g.xsz_buy_etf]

    # 4. 计算近期量能放大（改名为 get_v4_volume_ratio，避开冲突）
    all_codes = df['code'].tolist()
    vol_ratio = get_v4_volume_ratio(context, all_codes, short_period=5, long_period=20)
    log.info(f"[V4] 量比计算完成，覆盖 {len(vol_ratio)} 只")

    # 5. 叠加条件：涨停≥5 且 量比>1.5
    candidates = []
    for _, row in df.iterrows():
        code = row['code']
        if limit_up_count.get(code, 0) >= 10 and vol_ratio.get(code, 0) > 1.5:
            candidates.append(row)

    log.info(f"[V4] 涨停≥10且量比>1.5: {len(candidates)} 只")
    if not candidates:
        return [g.xsz_buy_etf]
    ...
    candidates.sort(
        key=lambda x: (
            -limit_up_count.get(x['code'], 0),
            x['market_cap'],
            x['pe_ratio'] if x['pe_ratio'] and x['pe_ratio'] > 0 else 999
        )
    )

    candidate_codes = [c['code'] for c in candidates[:g.xsz_stock_num * 5]]
    log.info(f"[V4] 排序后取前 {len(candidate_codes)} 只进入审计")

    # 7. 原有排雷 + 分红 + 冷却
    current_date = context.current_dt.date()
    start_audit_date = datetime.date(2025, 1, 1)
    if current_date > start_audit_date:
        audited_list = apply_nine_point_audit(context, candidate_codes)
    else:
        audited_list = filter_audit(context, candidate_codes)

    final_list = bonus_filter(context, audited_list)
    final_list = filter_cooldown_stocks(context, final_list)
    log.info(f"[V4] 排雷+冷却后: {final_list}")
    ...
    # 8. 价格与持仓
    last_prices = history(1, unit="1d", field="close", security_list=final_list)
    buy_list = [s for s in final_list if s in g.strategy_holdings[1] or last_prices[s][-1] <= 50][: g.xsz_stock_num]
    log.info(f"[V4] 最终买入列表: {buy_list}")
    return buy_list


# ====================== 涨停统计（逐只） ======================
def get_limit_up_count_batch(context, stock_list, days=250):
    count_dict = {}
    if not stock_list:
        return count_dict
    end_date = context.previous_date
    ...
    for stock in stock_list:
        try:
            df = get_price(
                stock, count=days, end_date=end_date,
                frequency='daily', fields=['close', 'pre_close'],
                skip_paused=True, fq='pre'
            )
            if df is None or len(df) == 0:
                continue
            ret = df['close'] / df['pre_close'] - 1
            count_dict[stock] = int((ret >= threshold).sum())
        except:
            continue
    return count_dict


# ====================== 近期量比计算（改名：避免覆盖原有 get_volume_ratio） ======================
def get_v4_volume_ratio(context, stock_list, short_period=5, long_period=20):
    """
    返回 dict {code: 量比}
    量比 = 近short_period日均成交量 / 前(long_period - short_period)日均成交量
    适用于小盘股近期放量判断，与原有ETF的 get_volume_ratio 完全隔离
    """
    ratio_dict = {}
    if not stock_list:
        return ratio_dict
    try:
        vol_dict = history(
            long_period, unit='1d', field='volume',
            security_list=stock_list, skip_paused=True,
            df=False, fq='pre'
        )
    except Exception as e:
        log.error(f"获取成交量数据失败：{e}")
        return ratio_dict

    for code in stock_list:
        if code not in vol_dict:
            continue
        vol_arr = vol_dict[code]
        if len(vol_arr) < long_period:
            continue
        short_avg = vol_arr[-short_period:].mean()
        long_avg = vol_arr[:-short_period].mean()
        if long_avg and long_avg != 0:
            ratio_dict[code] = short_avg / long_avg
    return ratio_dict
...
def after_code_changed(context):
    unschedule_all()
    # ========== 时间调控 start ==========
    GTR = -1  # Global Time Regulation：负数=提前(分钟)，正数=延后，0=不变

    def tr(default_time, strategy, ic=False):
        """Time Regulate：根据策略和默认时间返回调控后的时间
        Args:
            default_time: 默认时间字符串，如 "09:40"
            strategy: 策略名称，如 '策略1'
            ic: Is Controlled是否受GTR统一调控，True=受控，False=原样返回
        """
        # 策略开关字典：策略名为True + ic=True，GTR才生效
        strategy_switch = {
            '策略1': True,
            '策略2': True,
            '策略3': True,
        ...
        # 必须同时满足：策略开关为True 且 ic=True，GTR才生效
        if not (strategy_switch.get(strategy, False) and ic):
            return default_time
    ...
    if g.portfolio_value_proportion[0] > 0:
        ...
        run_weekly(strategy_1_sell, 2, tr("09:40", '策略1', ic=True))
        run_weekly(strategy_1_buy, 2, tr("09:45", '策略1', ic=True))
        run_daily(sell_small_cap_stocks, time=tr("10:00", '策略1', ic=True))
        # ATR止损价日常更新
        if g.enable_atr_stop_loss:
            run_daily(update_atr_stop_prices, tr("10:30", '策略1', ic=True))
            run_daily(update_atr_stop_prices, tr("14:00", '策略1', ic=True))
        if g.huanshou_check:
            run_daily(check_small_cap_turnover, tr("10:30", '策略1', ic=True))
        run_daily(check_small_cap_limit_up, tr("14:00", '策略1', ic=True))
        if g.check_defense:
            run_daily(check_defense_trigger, tr("14:50", '策略1', ic=True))
        run_daily(close_account, tr("14:50", '策略1', ic=True))

    # 策略2 ETF反弹策略
    if g.strategy_ETF_2000_proportion > 0:
        run_daily(capital_balance_2, tr("14:45", '策略2', ic=True))
        run_daily(strategy_2_sell, tr("14:49", '策略2', ic=True))
        run_daily(strategy_2_buy, tr("14:50", '策略2', ic=True))
    ...
    if g.portfolio_value_proportion[2] > 0:
        if "morning_routine" not in globals() or "afternoon_routine" not in globals():
            ...
            run_daily(afternoon_routine, time=tr("13:10", '策略3', ic=True))
            run_daily(reset_daily_flags, time=tr("15:10", '策略3', ic=True))
    ...
    if g.portfolio_value_proportion[3] > 0:
        ...
        run_monthly(adjust_blue_chip_position, 1, time=tr("10:40", '策略4', ic=True))
