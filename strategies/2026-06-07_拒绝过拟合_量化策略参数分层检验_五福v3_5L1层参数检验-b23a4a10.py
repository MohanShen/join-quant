...
# title: 拒绝过拟合！量化策略参数分层检验—五福v3.5L1层参数检验
...
def initialize(context):
    ...
    """ L1参数测试：L1级别参数都关闭，测试21-24周期 """
    # 滤波器参数 — 保持默认，L4开始扫描 滤波器参数（正常期使用拉普拉斯，震荡期使用高斯）
    g.laplace_s_param = 0.05             # 拉普拉斯衰减率
    g.laplace_min_slope = 0.002          # 拉普拉斯斜率阈值
    g.gaussian_sigma = 1.2              # 高斯标准差 默认值：1.2
    ...
    # 过滤器阈值 — 保持默认，L3开始扫描 过滤开关及参数
    g.r2_threshold = 0.4                 # R²阈值
    g.volume_lookback = 5                # 成交量回看天数（交易日） 默认值 5
    g.volume_threshold = 1.8             # 成交量比阈值 默认值 1.8
    g.loss = 0.97                        # 单日最大允许跌幅（3%） 默认值 0.97 
...
def execute_buy_trades(context):
    ...
    if num_etfs_to_buy <= 0:
        log.info(f"当前实际持仓数量({actual_holding_count})已达到或超过目标({g.holdings_num})，无需买��")
        log.info("========== 买入操作完成 ==========")
        return
