...
这里的最优组合是按近一段时间收益最大的组合为结果。
后期将计划添加以收益加权最优组合计算均线组合，并添加自动选股功，请期待！

策略说明：
所谓的三进兵是指三条EMA均线组合的策略
交易原则：
...
def initialize(context):
    ...
    # 设置成交量比例
    set_option('order_volume_ratio', 0.33)
    # 日志过滤
    ...
    # 交易股票池
    g.stokcs_pool = ['603000.XSHG', '600776.XSHG', '000859.XSHE']
    # 最大持仓量
    g.hold_count = 3
    # 首次计算均线组合
...
def trade_func(context):
    ...
    # 整合日期
    days = get_trade_days(end_date=context.current_dt.date(), count=3)
    yesterday = days[-2]
