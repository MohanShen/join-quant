...
# postId: fbe8656d80c4a6ad79053deed9a28d05
# backtestId: 8124e384c83faf16be46fe46a5a337bc
# title: 五福51_三状态V2_正常震荡三ETF分仓_走弱一ETF

# ============================================================
# 【本次修改说明：正常/震荡期三 ETF 分仓版】
# 修改日期：2026-05-12
# 修改目标：将原策略“集中持有 1 只 ETF”调整为：
#   1）正常期：最多分仓持有 3 只 ETF；
#   2）震荡期：最多分仓持有 3 只 ETF；
#   3）走弱期：仍集中持有 1 只 ETF，优先使用原策略的全球/海外/商品池或防御 ETF 逻辑；
#   4）买入函数由“只用剩余现金补买缺失 ETF”升级为“目标 ETF 等权再平衡”，
#      避免原来满仓 1 只 ETF 时，虽然目标变为 Top3，但资金仍无法自动分散到 3 只的问题；
#   5）新增等权再平衡缓冲参数 g.equal_weight_rebalance_buffer，默认 5%，
#      当持仓市值与目标市值偏离小于 5% 时不强行调仓，以减少无意义换手；
#   6）不改变原策略的三态市场判断、ETF 池、动量排序、过滤条件、止损和回撤风控主逻辑。
# ============================================================
...
#行业ETF：
    ...
    # ==================== 持仓数量控制 ====================
    # 本次修改：正常期、震荡期各分仓持有 3 只 ETF；走弱期仍集中持有 1 只 ETF。
    # 注意：g.holdings_num 会在 refresh_holdings_num_by_regime(context) 中按市场状态动态刷新。
    g.holdings_num = 3
    g.holdings_num_normal = 3
    g.holdings_num_oscillation = 3
    g.holdings_num_weak = 1
    g.defensive_etf = "511880.XSHG"
    g.min_money = 10
    # 等权再平衡缓冲带：持仓市值与目标市值偏离小于 5% 时不强制调仓，减少无意义换手。
    g.equal_weight_rebalance_buffer = 0.05
    g.target_etfs_list = []
    g.trade_entry_open = {}
...
def explain_filter_failures_for_etf(etf_code, metrics=None):
    ...
    try:
        ...
        # 2) R² 过滤（走弱期默认不启用）
        if getattr(g, 'enable_r2_filter', False) and regime != '走弱期':
        ...
        # 3) 走弱期均线过滤
        if getattr(g, 'enable_ma_filter', False) and regime == '走弱期':
        ...
        # 5) 短期风控（三日单日跌幅）
        if getattr(g, 'enable_loss_filter', False):
        ...
        # 7) 正常期拉普拉斯
        if getattr(g, 'enable_laplace_filter', False) and regime == '正常期':
            if not m.get('passed_laplace', True):
                slope = m.get('laplace_slope')
                min_s = float(getattr(g, 'laplace_min_slope', 0))
                ...
                if security not in candidate_codes_today:
                    ...
                    if security not in filtered_codes_today:
                        ...
                        if th is not None and sc is not None:
                            ...
                            detail = f"通过过滤但未入候选池：过滤后排名第{rk}（���Top10参与候选池）"
                        else:
...
def _get_position_amount_value(context, security):
    """
    返回某只标的当前持仓数量和按场内 last_price 重估的持仓市值。
    用于等权再平衡，避免仅使用 position.value 在盘中可能滞后的问题。
    """
    try:
...
def smart_rebalance_target_value(security, target_value, context, exit_reason='等权再平衡'):
    """
    专用于“多 ETF 等权分仓”的目标市值调仓函数。

    为什么不直接复用 smart_order_target_value？
    - 原 smart_order_target_value 在 target_value>0 时会先用 available_cash 限制 target_value；
    - 如果原来满仓 1 只 ETF，现在要把它降到 1/3 仓位，available_cash 可能为 0，
      原函数会把 target_value 截成 0，导致无法正确“减到目标市值”；
    - 所以这里单独写一个再平衡函数：先按目标市值计算目标股数，再判断是买入还是卖出。
    """
    try:
    ...
    try:
        ...
        if cd.paused:
            log.info(f"{security} {security_name}: 今日停牌，跳过再平衡")
            return False
        current_price = float(cd.last_price or 0)
        if current_price <= 0:
            log.info(f"{security} {security_name}: 当前价格无效，跳过再平衡")
            return False
        if current_price >= cd.high_limit:
            log.info(f"{security} {security_name}: 当前涨停，跳过再平衡")
            return False
        if current_price <= cd.low_limit:
            log.info(f"{security} {security_name}: 当前跌停，跳过再平衡")
            return False
    except Exception as e:
        log.warning(f"{security} {security_name}: 获取行情失败，跳过再平衡，原因: {e}")
        return False

    # ---------- 2. 计算目标股数与差额 ----------
    position = context.portfolio.positions.get(security, None)
    current_amount = int(position.total_amount) if position and position.total_amount > 0 else 0

    # ETF 按 100 股一手取整；目标金额太小则目标股数为 0。
    ...
    if amount_diff == 0:
        log.info(f"{security} {security_name}: 当前持仓已接近目标股数，无需再平衡")
        return True

    if 0 < trade_value < g.min_money:
        log.info(f"{security} {security_name}: 再平衡交易金额{trade_value:.2f}小于最小交易额{g.min_money}，跳过")
        return False

    # ---------- 3. 买入方向：按可用现金兜底 ----------
    if amount_diff > 0:
        available_cash = float(context.portfolio.available_cash or 0)
        if available_cash < g.min_money:
            log.info(f"{security} {security_name}: 可用现金{available_cash:.2f}不足，无法买入再平衡")
            return False

        # 买入估算成本：佣金 + 滑点，防止实际下单金额超过现金。
        buy_commission_rate = 0.0001
        slippage_rate = 0.0001
        estimated_price = current_price * (1 + buy_commission_rate + slippage_rate)
        max_buy_amount = int(available_cash / estimated_price)
        max_buy_amount = (max_buy_amount // 100) * 100

        if max_buy_amount <= 0:
            log.info(f"{security} {security_name}: 现金不足买入 100 股，跳过")
            return False

        if amount_diff > max_buy_amount:
            log.info(
                f"{security} {security_name}: 现金最多可买{max_buy_amount}股，"
                f"原计划买入{amount_diff}股，已按现金上限调低"
            )
            amount_diff = max_buy_amount

    # ---------- 4. 卖出方向：遵守 T+1 可卖数量 ----------
    if amount_diff < 0:
        closeable_amount = int(position.closeable_amount) if position else 0
        if closeable_amount <= 0:
            log.info(f"{security} {security_name}: 当天买入不可卖出或无可卖数量，暂不能再平衡减仓")
            return False
        sell_amount = min(abs(amount_diff), closeable_amount)
        sell_amount = (sell_amount // 100) * 100
        if sell_amount <= 0:
            log.info(f"{security} {security_name}: 可卖数量不足 100 股，跳过再平衡减仓")
            return False
        amount_diff = -sell_amount
    ...
    if order_result:
        if amount_diff > 0:
            log.info(
                f"📦 【等权再平衡买入】{security} {security_name}，数量: {amount_diff}，"
                f"价格: {current_price:.3f}，目标市值: {target_value:.2f}"
            )
        else:
            log.info(
                f"📤 【等权再平衡减仓】{security} {security_name}，数量: {abs(amount_diff)}，"
                f"价格: {current_price:.3f}，目标市值: {target_value:.2f}，原因: {exit_reason}"
            )
        return True

    log.info(f"{security} {security_name}: 再平衡下单失败")
    return False
...
def execute_buy_trades(context):
    ...
    本次修改重点：
    1）目标 ETF 数量由 g.holdings_num 控制；正常期/震荡期为 3，走弱期为 1；
    2）不再只是把剩余现金平均买入缺失 ETF，而是对目标 ETF 做等权目标市值配置；
    3）若原来满仓 1 只 ETF，且该 ETF 仍在 Top3 里，会先把它减到约 1/3 仓，再买入另外目标 ETF；
    4）设置 5% 再平衡缓冲，偏离不大时不调仓，降低换手和滑点冲击。
    """
    log.info("========== 买入/等权再平衡操作开始 ==========")

    raw_targets = list(getattr(g, 'target_etfs_list', []) or [])
    target_count = max(1, int(getattr(g, 'holdings_num', 1)))
    target_etfs = raw_targets[:target_count]

    if not target_etfs:
        log.info("根据计算的结果，今日无目标ETF，保持空仓")
        log.info("========== 买入/等权再平衡操作完成 ==========")
        return
    ...
    # 止损冷却只作用于“准备新买入”的 ETF；已经持有的目标 ETF 不因冷却而强制卖出。
    new_targets = [etf for etf in target_etfs if etf not in current_holdings]
    new_targets_after_cooldown = _filter_stop_loss_rebuy_cooldown(context, new_targets)
    blocked_targets = set(new_targets) - set(new_targets_after_cooldown)
    if blocked_targets:
        log.info(f"【止损冷却】以下新目标 ETF 暂不买回: {list(blocked_targets)}")
        target_etfs = [etf for etf in target_etfs if etf not in blocked_targets]

    if not target_etfs:
        log.info("目标ETF全部被止损冷却过滤，今日不买入")
        log.info("========== 买入/等权再平衡操作完成 ==========")
        return
    ...
    if total_value <= 0:
        log.warning("组合总资产异常，无法执行等权再平衡")
        log.info("========== 买入/等权再平衡操作完成 ==========")
        return

    target_value_each = total_value / len(target_etfs)
    rebalance_buffer = float(getattr(g, 'equal_weight_rebalance_buffer', 0.05))
    rebalance_buffer = max(0.0, rebalance_buffer)

    target_desc = ", ".join(target_etfs)
    log.info(
        f"【等权目标】市场状态={getattr(g, 'market_regime', '未知')}，"
        f"目标ETF数量={len(target_etfs)}，目标ETF={target_desc}，"
        f"组合总资产={total_value:.2f}，单只目标市值≈{target_value_each:.2f}，"
        f"再平衡缓冲={rebalance_buffer:.1%}"
    )

    # ---------- 第一步：先减掉目标ETF中的超配仓位，释放现金 ----------
    for etf in target_etfs:
        _, current_value = _get_position_amount_value(context, etf)
        if current_value <= 0:
            continue
        if current_value > target_value_each * (1 + rebalance_buffer):
            log.info(
                f"【等权再平衡】{etf} 当前市值{current_value:.2f} > 目标上沿"
                f"{target_value_each * (1 + rebalance_buffer):.2f}，执行减仓到目标市值"
            )
            smart_rebalance_target_value(
                etf,
                target_value_each,
                context,
                exit_reason='目标ETF等权再平衡减仓'
            )
        else:
            log.info(
                f"【等权再平衡】{etf} 当前市值{current_value:.2f} 未超过目标上沿，暂不减仓"
            )

    # ---------- 第二步：买入缺失或低配的目标ETF ----------
    for etf in target_etfs:
        _, current_value = _get_position_amount_value(context, etf)
        lower_bound = target_value_each * (1 - rebalance_buffer)

        if current_value >= lower_bound:
            log.info(
                f"【等权再平衡】{etf} 当前市值{current_value:.2f} 已达到目标下沿"
                f"{lower_bound:.2f}，无需补仓"
            )
            continue

        if context.portfolio.available_cash < g.min_money:
            log.info(
                f"可用现金 {context.portfolio.available_cash:.2f} 不足最小交易额 {g.min_money:.2f}，停止补仓"
            )
            break

        success = smart_rebalance_target_value(
            etf,
            target_value_each,
            context,
            exit_reason='目标ETF等权再平衡补仓'
        )
        if success:
            log.info(f"✅ ETF {etf} 等权目标调仓完成")
            if current_value <= 0:
                record_buy_trade_entry(context, etf)
        else:
            log.info(f"❌ ETF {etf} 等权目标调仓失败")
...
def _stop_loss_rebuy_cutoff_dt():
    s = getattr(g, 'stop_loss_rebuy_cutoff_time', '13:10')
    try:
