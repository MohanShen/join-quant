...
//   node -e "process.argv[2]='study_ETF溢价_baseline-e6__train__e6';require('./study/ETF溢价/yearsplit.js')"
const fs = require('fs');
...
for (const [y, rows] of Object.entries(byYear)) {
  ...
  console.log(`${y}\tdays=${rows.length}\ttotal=${(total * 100).toFixed(2)}\tmaxdd=${(mdd * 100).toFixed(2)}\tvol=${(sd * Math.sqrt(250)).toFixed(4)}\tsharpe~${sharpe.toFixed(2)}\tup-days=${up}`);
}
