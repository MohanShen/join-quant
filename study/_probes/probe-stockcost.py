...
def initialize(context):
    ...
    # ── PROBE (stock-cost precedence) ────────────────────────────────────────
    # A 5%/side stock commission. If the harness OVERRIDE's set_commission really re-pins
    # stock costs, this is neutralised and the result matches the control. If the author's
    # set_order_cost(type='stock') governs, returns collapse. The effect size is enormous on
    # purpose, so no control-vs-noise judgement is needed — the control is int-001's own
    # measured row (annual 28.28%, sharpe 1.57, maxdd 10.97%).
    set_order_cost(OrderCost(open_commission=0.05, close_commission=0.05,
                             close_tax=0, min_commission=5), type='stock')
