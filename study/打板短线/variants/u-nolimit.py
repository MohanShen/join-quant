...
def get_hl_stock(stock_list, date1, days=1):
    ...
    h_s = get_price(stock_list, end_date=date1, frequency='daily', fields=['close', 'high_limit', 'pre_close', 'paused'],  # u-nolimit
                  count=days, panel=False, fill_paused=False, skip_paused=False
                  ).query('close < high_limit and close >= 1.07*pre_close and paused==0').groupby('code').size()  # u-nolimit: +7% but not limit-up
...
def get_ever_hl_stock2(stock_list, date1):
    if not stock_list:return []
    h_s = get_price(stock_list, end_date=date1, frequency='daily', fields=['close', 'high', 'high_limit', 'pre_close', 'paused'],  # u-nolimit
                  count=1, panel=False, fill_paused=False, skip_paused=False
                  ).query('high >= 1.07*pre_close and close < 1.07*pre_close and paused==0').groupby('code').size()  # u-nolimit: touched +7%, closed below
    return h_s.index.tolist()
