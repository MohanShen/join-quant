...
# title: 低换手红利策略
# ---
# variant q-7 of study base snapshot — family: 红利低频
...
def get_stock_pool(context):
    ...
    # q-7: drop listings younger than 250 days
    all_stocks = all_stocks[[(pd.Timestamp(curr_date) - pd.Timestamp(d)).days >= 250 for d in all_stocks['start_date']]]
    ...
    # q-7: drop ST / *ST / delisting names
    stock_list = [s for s in stock_list if not current_data[s].is_st and 'ST' not in current_data[s].name and '退' not in current_data[s].name]
...
def calc_factor_rank(context, stock_list):
    ...
    # 综合排名 = 加权排名之和，越小越好
    df['total_rank'] = df['dy_rank'] * 3  # q-7: turnover rank removed (as q-1)
