...
def get_dividend_yield_ttm(stock_list, date):
    ...
    # [idea-2] TRUE universe widening. idea-1 measured EXACTLY ZERO change because the
    # restriction was never the `dy_ttm > 0` line: this Series is built from the dividend
    # RECORDS table, so a non-payer never enters it and the downstream dropna() drops it.
    # Reindexing over the full stock_list with 0 is what actually widens the pool — and it is
    # what makes idea-1's commented-out filter load-bearing, since those zeros would otherwise
    # be filtered straight back out.
    return pd.Series(dy_dict).reindex(stock_list).fillna(0.0)
...
def calc_factor_rank(context, stock_list):
    ...
    df = df.dropna()
    # 排除股息率为0或负的
    # [idea-1] universe widened: q-3b held this line and measured 'low-turnover AMONG
    # DIVIDEND PAYERS'. This arm removes the dividend gate so the same turnover rank runs
    # on the full A-share pool. One line changed vs q-3b; everything else identical.
    # df = df[df['dy_ttm'] > 0]
    ...
    # 综合排名 = 加权排名之和，越小越好
    df['total_rank'] = df['turnover_rank']
