...
# title: 低换手红利策略
# ---
# variant q-8a of study base snapshot — family: 红利低频
...
def calc_factor_rank(context, stock_list):
    ...
    # 排除股息率为0或负的
    df = df[df['dy_ttm'] > 0]
    # q-8a: keep only the LOWEST 20d-turnover tercile of payers
    df = df[df['turnover_20d'] <= df['turnover_20d'].quantile(1.0/3)]
    ...
    # 综合排名 = 加权排名之和，越小越好
    df['total_rank'] = df['dy_rank'] * 3  # q-8a: yield rank only, inside the tercile
