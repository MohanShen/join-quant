...
def initialize(context):
    ...
    log.info("【五福闹新春】v5.0启动！[DeepseekV4 v32: v26+正常期Laplace s=0.05→0.07]")
...
#行业ETF：
    ...
    g.laplace_s_param_normal = 0.07  # v32: 正常期略微放宽
    g.laplace_s_param_weak = 0.10  # v10: 走弱期更宽松
    ...
    # v2: 去除分钟级止损（v3.5研究核心发现：紧止损=Alpha杀手）
...
def update_sector_pool(context):
    ...
    log.info(f"【动态池更���】特别组分布: {group_counts}")
...
def calculate_all_metrics_for_etf(etf, etf_name, hist_closes, hist_volumes, current_price, today_vol, context):
    try:
        ...
        if len(price_series) >= 10:
            try:
                laplace_s = g.laplace_s_param_weak if g.is_a_share_weak else g.laplace_s_param_normal  # v32: 正常期使用0.07
                laplace_values = laplace_filter(price_series, s=laplace_s)
        ...
        return {
            ...
            'passed_r2': passed_r2,
            'passed_ma': passed_ma,
            'passed_volume': volume_ratio is not None,  # q-1b: ablate volume THRESHOLD, keep None-data guard (avoids the base's volume-off NameError)
...
def apply_filters(metrics_list):
    steps = [
        ...
        ('R²', lambda m: m['passed_r2'], g.enable_r2_filter),  # v6: 走弱期也启用R²
        ('均线', lambda m: m['passed_ma'], g.enable_ma_filter and not g.is_a_share_weak),  # v26: 走弱期关闭MA
        ('成交量', lambda m: m['passed_volume'], g.enable_volume_check and not g.is_a_share_weak),  # v26: 走弱期关闭成交量
        ('短期风控', lambda m: m['passed_loss'], g.enable_loss_filter and not g.is_a_share_weak),  # v26: 走弱期关闭短期风控
        ('溢价率', lambda m: m['passed_premium'], g.enable_premium_filter and not g.is_a_share_weak),  # v26: 走弱期关闭溢价率
        ('拉普拉斯滤波', lambda m: m['passed_laplace'], g.enable_laplace_filter and not g.is_a_share_weak),  # v26: 走弱期关闭拉普拉斯
    ]
...
# ===== AUTOENHANCE NORMALIZATION OVERRIDE (appended; strategies/ file untouched) =====
