...
def apply_filters(metrics_list):
    steps = [
        ...
        ('均线', lambda m: m['passed_ma'], g.enable_ma_filter),  # q-3: 走弱期不再关闭MA
        ('成交量', lambda m: m['passed_volume'], g.enable_volume_check),  # q-3: 走弱期不再关闭成交量
        ('短期风控', lambda m: m['passed_loss'], g.enable_loss_filter),  # q-3: 走弱期不再关闭短期风控
        ('溢价率', lambda m: m['passed_premium'], g.enable_premium_filter),  # q-3: 走弱期不再关闭溢价率
        ('拉普拉斯滤波', lambda m: m['passed_laplace'], g.enable_laplace_filter),  # q-3: 走弱期不再关闭拉普拉斯
    ]
