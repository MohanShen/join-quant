...
def update_sector_pool(context):
    ...
    log.info(f"【动态池更新】进入���别组: {len(special_etfs)}只")
...
def record_daily_positions_to_storage(context):
    ...
    try:
        ...
        log.error(f"记录持仓失败: {e}")

def output_all_positions_summary(context):
    """在回测最后一天，汇总输出整个回测期间的所有持仓记录"""
    end_date = context.run_params.end_date
    today = context.current_dt.date()
    if today != end_date:
        return
    if not hasattr(g, 'all_positions_records') or not g.all_positions_records:
        log.info("【持仓汇总】回测期间无任何持仓记录")
        return
    log.info("")
    log.info("=" * 80)
    log.info("【回测期间每日持仓成交金额汇总表】")
    log.info("=" * 80)
    log.info(f"{'日期':<12} {'当日成交金额':<15} {'持仓ETF代码':<15} {'持仓ETF名称':<25}")
    log.info("-" * 80)
    for r in g.all_positions_records:
        log.info(f"{r['date']:<12} {r['turnover']:<15} {r['code']:<15} {r['name']:<25}")
    log.info("=" * 80)
    log.info(f"共 {len(g.all_positions_records)} 条记录")
    log.info("")