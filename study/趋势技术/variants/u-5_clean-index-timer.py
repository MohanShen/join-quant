...
def calculate_signals(context):
    ...
    try:
        # 1. 大盘趋势
        g.market_trend_bull = check_market_trend(context.previous_date)
        ...
        df_close = get_batch_close_prices(stock_pool, context.previous_date, total_days_needed)
        if df_close.empty:
        ...
        g.buy_list = [{'stock': '510300.XSHG', 'day_rps': 0.0, 'week_rps': 0.0, 'month_rps': 0.0, 'slope': 0.0}]  # u-5: index ETF control
...
def market_open(context):
    ...
    for position in context.portfolio.positions.values():
        ...
        rps = day_rps.get(stock, calculate_single_rps(stock, context.previous_date, g.N))
    ...
    if not g.market_trend_bull:  # u-5: explicit timer, cash when close <= MA20
