...
#1-7 止盈止损
# 在sell_stocks函数中加入ATR止损逻辑
def sell_stocks(context):
    if g.run_stoploss:
        current_positions = context.portfolio.positions

        # 计算ATR止损
        atr_period = 14  # ATR周期，通常取14天
        atr_multiplier = 2  # ATR倍数，通常取2倍

        for stock in current_positions.keys():
            # 获取股票的ATR值
            history_data = get_price(stock, end_date=context.previous_date, frequency='daily', fields=['high', 'low', 'close'], count=atr_period + 1, panel=False)
            high = history_data['high'].values
            low = history_data['low'].values
            close = history_data['close'].shift(1).values
            ...
            if current_price <= stop_loss_price:
                ...
                log.debug("ATR止损, 卖出{}".format(stock))
...
def adjust_stock_num(context):
    if g.adjust_num is True:
        ...
        # 根据账户的总价值和单个股票的最大持仓比例来限制单个股票的持仓
