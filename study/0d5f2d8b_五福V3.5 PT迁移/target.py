...
# title: 五福闹新春V3.5 全网还原度最高的PT迁移版v1附回测截图
...
def initialize(context):
    ...
    run_daily(afternoon_routine_for_sell, time='13:10')
    run_daily(afternoon_routine_for_buy, time='13:15')
...
def morning_routine(context):
    """晨间准备流水线(09:00执行): 回撤监控 → 流动性阈值计算 → 动态池更新 → 固定池过滤 → 合并池"""
    log.info("[晨间交易流水线] 启动...")
...
def afternoon_routine_for_sell(context):
    """午后交易流水线(13:10执行): 震荡期退出检查 → 震荡期进入检查 → 动量计算 → 卖出执行"""
    log.info("[午后交易流水线] 启动...")
    log.info("[震荡期退出检查] 检查是否需要退出震荡期...")
    check_and_exit_range_bound_mode(context)     
    log.info("[震荡期进入检查] 检查是否需要进入震荡期...")
...
def afternoon_routine_for_buy(context):
    """午后交易流水线(13:10执行): 买入执行"""
    log.info("[买入执行] 执行买入操作...")
    execute_buy_trades(context)
    log.info("[午后交易流水线] 执行完毕! ")

def reset_daily_flags(context):
    """收盘后重置当日标志(15:10执行): 重置价格缓存 → 更新震荡期统计"""
    # 重置价格缓存(防止次日止损计算使用错误数据)
    g.cache_date = None
    g.yesterday_close_cache = {}
    log.info("[收盘重置] 收盘缓存重置完成")
    # 更新震荡期交易日计数
    ...
    try:
        ...
        if g.max_portfolio_value > 0:
            ...
            if current_drawdown >= g.drawdown_threshold:
                ...
                log.info(f"[回撤预警]")
                log.info(f"回撤达到 {current_drawdown:.2%} (阈值: {g.drawdown_threshold:.0%})")
...
def update_sector_pool(context):
    ...
    fund_companies = sorted(list(set([
        ...
        '兴证全球', '浙商', '诺安', '���海开源', '泰康', '泰达宏利', '农银汇理', '交银', '东方红', '财通', '华商',
    ...
    # 行业去重(普通组)
    normal_industry_groups = defaultdict(list)
    ...
    # 行业去重(特别组)    
    special_industry_groups = defaultdict(list)
...
def daily_merge_etf_pools(context):
    """每日合并固定池和动态池"""
    merged_etf_pool = list(set(g.filtered_fixed_pool + g.dynamic_etf_pool))
    merged_etf_pool.sort()
    log.info("[合并ETF池] 开始执行")
    log.info(f"[合并池统计] 固定池: {len(g.filtered_fixed_pool)}只, 动态池: {len(g.dynamic_etf_pool)}只, 合并后: {len(merged_etf_pool)}只")
    g.merged_etf_pool = merged_etf_pool
        ...
        if df is not None and len(df) >= max(g.ma_period, g.lookback_high_low_days):
            ...
            if g.enable_bias_trigger:
                ...
                if bias > g.bias_threshold:
                    ...
                    log.info(f"[条件触发] 乖离率: {bias:.2%} (阈值>{g.bias_threshold:.0%})")
            # 条件2: RSI超买回落
...
def execute_sell_trades(context):
    ...
    if ranked_etfs:
        ...
        if check_defensive_etf_available(context):
            ...
            log.info(f"确定最终目标(防御模式): {g.defensive_etf} {etf_name}")
        else:
            log.info("无最终目标(空仓模式)")
            target_etfs = []
