...
def initialize(context):
    ...
    __pool = get_index_stocks('399006.XSHE', date='2021-12-31')  # u-1bc: pre-registered pool rule, ranks 6-10 by cap, data ends before TRAIN
    __pool_df = get_fundamentals(query(valuation.code, valuation.market_cap).filter(valuation.code.in_(__pool)).order_by(valuation.market_cap.desc()).limit(10), date='2021-12-31')
    g.stokcs_pool = list(__pool_df['code'])[5:10]
    g.hold_count = 5
    run_daily(trade_func, 'open') 

def trade_func(context):
    # u-1bc: buy-and-hold control on the u-1 pool (q-1 hold form: equal weight on the first bar, never trade again)
