...
def initialize(context):
    ...
    __pool = get_index_stocks('399006.XSHE', date='2021-12-31')  # u-1c: pre-registered pool rule, data ends before TRAIN
    __pool_df = get_fundamentals(query(valuation.code, valuation.market_cap).filter(valuation.code.in_(__pool)).order_by(valuation.market_cap.desc()).limit(5), date='2021-12-31')
