// Usage: node -e "process.argv[2]='variants/u-1_pool-transfer.py';process.argv[3]='sjb-u-1';require('./study/三进兵/run.js')"
// One TRAIN backtest, foreground/blocking, output to data/autostudy-logs/<expId>.log, terminated
// by an EXIT line. Paths are relative to study/三进兵/ unless they start with enhance/.
const fs = require('fs');
const path = require('path');
const { spawnSync } = require('child_process');
const rel = process.argv[2], id = process.argv[3];
const file = rel.startsWith('enhance/') ? rel : path.join('study', '三进兵', rel);
...
fs.writeSync(log, `\nEXIT ${r.status}\n`);
console.log(fs.readFileSync(logPath, 'utf8').split('\n').filter(l => /^(SUMMARY|SERIES|EXIT)|usage|STOP|error|algorithmId|backtestId|回测完成/i.test(l)).join('\n'));
