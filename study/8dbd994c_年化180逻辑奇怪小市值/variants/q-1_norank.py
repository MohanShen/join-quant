...
def initialize(context):
    ...
    # 配置参数
    g.avoid_months = []        # 可设置跳过某些月份（如财报季避开1月、4月等）
...
def is_eligible_for_buy(context, stock, data_today, history):
    ...
    # 条件3：今日开盘价 < 昨日最低价 且 开盘跌幅 > 0.75%
    if not (today_open < prev_low and chg < -0.75):
        return False
...
def buy(context):
    ...
    for stock in buy_candidates:
        ...
        a1_p = getattr(tick, 'a1_p', None)
        a1_v = getattr(tick, 'a1_v', None)
    ...
    # 按 (买一档金额 / 总市值) 降序排序
    # [q-1 ablation] Ranking neutralized: keep valid_stocks in append order
    # (market-cap-ascending pool order). get_ratio remains defined but unused.
